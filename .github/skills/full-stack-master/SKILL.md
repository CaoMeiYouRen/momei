...
2. **D (Do) - 开发实现**
    ...
    2. **基础设施例外**：若事项涉及 Docker、CI/CD、部署配置、构建环境或运维脚本，应在全栈主流程内按需复用 `devops-specialist`，而不是额外拆出并行主责角色。
    3. **拒绝静默膨胀**：开发过程中发现新的想法、优化点或非阻塞缺陷时，必须返回 P 阶段重新分流，不得直接扩写当前任务。
    4. **自检修复**：开发完成通过 `code-quality-auditor` 消除 Lint 和类型报错。
    - **技能**：`database-expert`、`backend-logic-expert`、`vue-frontend-expert`、`devops-specialist`、`code-quality-auditor`
