...
### 阶段 1：P (Plan) - 需求分析与规划
1.  **范围闸门**：先核对事项是否已经属于当前待办、当前验收标准或当前阶段规划；若只是当前任务收口，不得误判为新增需求。
2.  **需求分析**：调用 `requirement-analyst` 理解需求并与用户核对意意。
3.  **插队分流**：若事项不在当前规划中，先判断它是允许插队的阻塞/高风险事项，还是应延期的 backlog；只有前者才允许继续推进。
4.  **任务拆解**：使用 `todo-manager` 分解任务并更新 `todo.md` 为 `in-progress`。
5.  **技术方案**：仅对允许执行的事项使用 `technical-planning` 生成文件修改清单。
...
4.  **拒绝静默膨胀**：开发过程中若发现额外问题或优化点，必须回到 P 阶段重新做准入判断，不得边做边扩大当前任务边界。

### 阶段 3：A (Audit) - 代码审计
1.  **安全审美**：调用 `code-auditor` 进行安全漏洞检查和项目规范对齐。

### 阶段 4：C1 (Commit) - 功能提交
1. **提交前检查**：确保所有代码变更都已通过质量审计（至少通过 lint 和 typecheck），且相关文档（如设计文档、API 文档）已更新。
2. **初次交付**：调用 `conventional-committer` 执行第一次 Git 提交（业务实现）。消息描述统一使用**中文**或用户的语言。
...
### 阶段 7：C2 (Commit) - 测试提交
1. **提交前检查**：确保所有代码变更都已通过质量审计（至少通过 lint 和 typecheck，及对应的 test），且相关文档（如设计文档、API 文档）已更新。
2.  **最终交付**：完成测试后执行第二次 Git 提交（测试增强）。消息描述统一使用**中文**或用户的语言。
...
4.  **范围守门**：在开发、测试、审计或验证阶段发现新增事项时，默认先延期判断，只有阻塞交付、功能回归、安全/合规高风险事项才允许插队。
5.  **安全执行**：在执行任何 `run_in_terminal` 前，严格核对 [AGENTS.md](../../AGENTS.md) 中的终端命令安全规范，确保路径校验和环境兼容。
