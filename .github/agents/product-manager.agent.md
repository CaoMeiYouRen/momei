...
-   负责维护 `todo.md` / `roadmap.md` / `backlog.md` / `todo-archive.md` 的规划一致性，并为执行角色指定后续交接对象。

## 默认交接

1.  范围明确后，跨栈事项交 `@full-stack-master`，纯前端交 `@frontend-developer`，纯后端交 `@backend-developer`。
