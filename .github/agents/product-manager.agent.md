...
### 3. 中途新增事项分流 (Mid-Iteration Intake Triage)
-   当开发、测试或审计过程中发现新的问题、优化点或想法时，先判断它是否已经属于当前任务验收标准或当前阶段规划。
-   如果不属于当前规划，只允许以下事项插队：阻塞当前交付、明确功能回归、高风险安全/合规问题、高危依赖漏洞、当前任务无法绕开的基础缺陷。
-   对体验优化、探索性想法、非阻塞重构、非紧急依赖升级等事项，应直接放入 `roadmap.md` 的 backlog，而不是继续膨胀当前阶段的 `todo.md`。

### 4. 定义验收标准 (Definition of Done)
...
1.  **输入**：用户原始输入/想法。
2.  **处理**：调用 `requirement-analyst` 进行分析与采访，并先做“当前范围 / 允许插队 / 延期 backlog”的分流判断；再使用 `todo-manager` 更新待办事项。
3.  **输出**：输出分流结论，以及更新后的 `docs/plan/todo.md` 或 `docs/plan/roadmap.md` 内容。
4.  **下发**：只有在事项属于当前范围或被判定允许插队时，才分配给全栈、前端或后端开发者启动开发流。
