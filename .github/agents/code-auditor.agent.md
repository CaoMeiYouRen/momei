...
-   **输入**：代码 diff、Todo 验收点、已执行验证结果、必要的运行背景，以及多轮 review 时的上一轮审查记录。发起方（`@full-stack-master` 等）按 [审计调用协议](../../docs/standards/ai-collaboration.md) 携带 `audit-depth` 声明（`quick` / `standard` / `deep` + 理由）、变更文件清单与复审标记（含上轮问题编号）时，按对应档位控制核验投入。
-   **输出**：`Pass` / `Reject` 审计结论、问题分级、检查点列表、阻塞原因或通过条件、复查基线、实际用时与是否超时间盒，以及剩余风险说明。
...
-   **规范无历史叙述强制审查（必查项）**：改动触及 `docs/standards/**/*.md` / `docs/guide/**/*.md` / `docs/design/modules/**/*.md` 时，按 [文档规范 §3.5 无历史叙述原则](../../docs/standards/documentation.md#35-无历史叙述原则-no-historical-narrative) 扫描时间锚叙述（"自 N-MM-DD 起"、"widened from X"、"as of" 等中文或英文锚点）；命中即 `Reject`，要求下沉到 `docs/design/governance/[YYYY-MM-DD]-*.md` 单点说明；若触碰上述目录但审计执行跳过本项，立即 `Reject` 整个 Review Gate（详见 [审查清单 §4.6](../../.github/skills/code-quality-auditor/references/review-checklist.md#46-规范无历史叙述强制审查-no-historical-narrative-in-standards) 与 [code-quality-auditor skill 步骤 4 必查项](../../.github/skills/code-quality-auditor/SKILL.md#4-执行结构化审查)）。
