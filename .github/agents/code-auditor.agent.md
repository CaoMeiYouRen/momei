...
-   **规范无历史叙述强制审查（必查项）**：改动触及 `docs/standards/**/*.md` / `docs/guide/**/*.md` / `docs/design/modules/**/*.md` 时，按 [文档规范 §3.5 无历史叙述原则](../../docs/standards/documentation.md#35-无历史叙述原则-no-historical-narrative) 扫描时间锚叙述（"自 N-MM-DD 起"、"widened from X"、"as of" 等中文或英文锚点）；命中即 `Reject`，要求下沉到 `docs/design/governance/<date>-*.md` 单点说明；若触碰上述目录但审计执行跳过本项，立即 `Reject` 整个 Review Gate（详见 [审查清单 §4.6](../../.github/skills/code-quality-auditor/references/review-checklist.md#46-规范无历史叙述强制审查-no-historical-narrative-in-standards) 与 [code-quality-auditor skill 步骤 4 必查项](../../.github/skills/code-quality-auditor/SKILL.md#4-执行结构化审查)）。
