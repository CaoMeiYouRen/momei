...
describe('db ready middleware', () => {
    ...
    it('should skip installation and static asset requests', async () => {
        ...
        expect(shouldWarmupDatabase('/api/settings/public')).toBe(false)
        expect(shouldWarmupDatabase('/api/posts/home')).toBe(false)
        expect(shouldWarmupDatabase('/api/categories')).toBe(false)
        expect(shouldWarmupDatabase('/api/tags')).toBe(false)
        expect(shouldWarmupDatabase('/api/admin/content-insights')).toBe(false)
        expect(shouldWarmupDatabase('/api/user/api-keys')).toBe(false)
        expect(shouldWarmupDatabase('/api/upload')).toBe(false)
        expect(shouldWarmupDatabase('/api/notifications')).toBe(false)
        ...
        await ensureRequestDatabaseReady(createEvent('/api/settings/public'))
        await ensureRequestDatabaseReady(createEvent('/api/posts/home'))
        await ensureRequestDatabaseReady(createEvent('/api/categories'))
        await ensureRequestDatabaseReady(createEvent('/api/tags'))
        await ensureRequestDatabaseReady(createEvent('/api/admin/content-insights'))
        await ensureRequestDatabaseReady(createEvent('/api/user/api-keys'))
        await ensureRequestDatabaseReady(createEvent('/api/upload'))
        await ensureRequestDatabaseReady(createEvent('/api/notifications'))
