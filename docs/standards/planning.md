...
-   **阶段升格去重原则**: 当某个 backlog 条目经过评估后被正式纳入新的 Phase、当前阶段待办或修复型 Hotfix 时，必须立即从 `roadmap.md` 的“未来规划与积压项”中移除，或显式改写为“已上收至某阶段”的历史说明；严禁同一事项同时出现在“当前/新阶段规划”和“未来规划与积压项”两处，避免重复统计与优先级混乱。
...
-   **功能需求管理原则**:
    ...
    -   **阶段提取后去重**: 一旦某项 backlog 内容已被提取并写入当前阶段或下一阶段的正式规划，必须同步从 backlog 删除，不能以“保留原始想法”为由继续重复保留。
