...
### 1.1 测试设计原则 (Test Design Principles)
...
4.  **测试规模评估**:
    ...
    -   若当前任务仍无法明确“哪条断言会在缺陷存在时失败”，说明测试目标尚未收敛，应先回到风险定义或需求澄清，而不是机械补测。
...
| :--- | :--- | :--- | :--- |
| 周级治理 | `pnpm regression:weekly` | 60 分钟以内 | 固定覆盖 coverage、依赖安全、文档事实源 / i18n 检查、文档长度守线、缺词 parity 审计、重复代码基线与脚本治理体检 |
| 发版前 | `pnpm regression:pre-release` | 90 分钟以内 | 固定覆盖 `release:check:full`（内含 `i18n:audit:missing`）、文档 i18n 检查、文档长度守线、性能预算与重复代码复核 |
| 阶段收口前 | `pnpm regression:phase-close` | 120 分钟以内 | 在发版前入口基础上补强 coverage、文档长度守线、strict 重复代码检查与 Review Gate 证据；`release:check:full` 内部继续阻断 i18n 缺词；日志窗口超限时直接阻断归档 |
