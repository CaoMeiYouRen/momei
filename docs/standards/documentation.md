...
4. 若阶段状态从“规划中 / 进行中”变为“已归档 / 已审计归档”，必须同步检查 `docs/i18n/*/plan/roadmap.md` 的摘要状态与 `last_sync`，但 `todo.md` 与 `backlog.md` 仍保持中文事实源，不为归档动作单独扩展翻译件。
5. 阶段归档块必须能独立说明“为什么可以归档”，至少包含审计结论、主线完成情况、关键验证或回归入口，以及残余风险或后续观察项；不得只写一句“已完成”。
