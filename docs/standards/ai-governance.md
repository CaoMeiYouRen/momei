...
5.  **先验来源核验（投毒面防御）**: 引入任何外部 skill / agent / MCP / 模板前，必须核对来源仓库 URL 与维护组织是否为可信主体，禁止仅凭"内容看起来有用"直接采纳——伪装成"有用文档 / 技能"诱导信任是已知投毒模式（TrustFall 教训）。来源不明的资产必须登记到准入清单并完成来源核验后才能引用，核验口径见 [安全规范 §5.2 供应链信任边界](./security.md#52-供应链信任边界-supply-chain-trust-boundary)。
