...
- **Stage 35 is now formally planned**: No new feature is being promoted in this phase. The Chinese source of truth has switched to a “0 new features + 5 optimizations” execution face, focused on AI task metering calibration and frontend-direct TTS regression hardening, PostgreSQL hot-read / connection-wakeup governance, the next narrow ESLint / type-debt slice, structural reuse governance (duplicate code, scattered types, pure helpers, and utility functions), and the next existing-code comment-governance slice.
