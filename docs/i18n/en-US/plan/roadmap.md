...
- **Stage 37 is now formally planned**: The next execution slice has been promoted in the Chinese source of truth and focuses on Windows local `nuxt dev` / `nuxt build` performance, high-risk test-effectiveness work, the next narrow ESLint / type-debt slice, at least three structural-reuse hotspots, and a PostgreSQL long-window review slice.
