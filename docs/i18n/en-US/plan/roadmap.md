...
- **Stage 67 has been registered (planned, not yet archived)**: First stage of the PrimeVue → caomei-ui UI component library migration — integration baseline and consumption path (exact-pinned npm `caomei-ui@0.2.0`, dual-library coexistence, same-name composables isolation, route-level whitelist carrier; the base layer entry moved from `styles.css` to `theme.css` in 0.2.0; the base landed on 0.1.0 and was re-anchored to 0.2.0 on 2026-09-24), a three-layer visual verification regression baseline (unit / E2E / screenshot), the global token semantic-layer bridge, and the B2 pilot page migration. Rationale and batch breakdown live in the [migration plan](../../../design/governance/2026-09-18-primevue-to-caomei-ui-migration-plan.md); follow-up batches are Stage 68 / 69.
