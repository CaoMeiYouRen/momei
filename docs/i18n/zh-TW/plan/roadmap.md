...
- **第二十五階段已正式上收**：中文 `roadmap.md` 與 `todo.md` 已將部署與初始化體驗、編輯器與正文渲染第二輪收口、構建速度與包體性能深化、Cloudflare 運行時相容研究，以及 AI 初始化 / 配置問答助手評估 5 個方向正式納入下一階段規劃。
- **翻譯頁範圍說明**：本頁保留近期已完成審計階段的摘要，以及當前已正式上收階段的簡要狀態；詳細驗收標準與任務拆解仍以中文 `roadmap.md`、`todo.md` 為唯一事實源。
