...
- **第二十一階段已完成審計歸檔**：UI 真實環境驗證流程治理、腳本入口收斂、標籤翻譯前置化，以及預設封面文案壓縮與大字體策略，均已在中文事實源收口。下一階段目前仍停留在候選分析，不會提前寫入正式 roadmap / todo。
- **第二十一階段主軸**：目前中文事實源中的四條主線為 UI 真實環境測試流程治理、腳本專項優化與執行入口治理、標籤翻譯前置化與正文翻譯解耦，以及預設封面生成文案壓縮與大字體策略優化。
- **翻譯頁範圍說明**：本頁現保留第十八至第二十階段的歸檔摘要與第二十一階段方向摘要；詳細驗收標準與任務拆解仍以中文 `roadmap.md`、`todo.md` 為唯一事實源。
