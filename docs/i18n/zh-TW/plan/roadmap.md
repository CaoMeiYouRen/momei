...
- **第十八階段已正式規劃**：重點收斂多引擎瀏覽器驗證與效能預算基線、`html-minifier` 高風險依賴鏈替換、後台 admin locale 大檔拆分、`ja-JP` 正式對齊，以及重複純函式 / 型別片段的首輪治理。
- **翻譯頁範圍說明**：本頁僅保留第十八階段摘要；詳細驗收標準與任務拆解仍以中文 `roadmap.md`、`todo.md` 為唯一事實源。
