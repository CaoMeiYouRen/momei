...
- **第二十一段階は監査完了でアーカイブ済みです**: UI 実環境テストフロー治理、スクリプト実行入口の整理、タグ翻訳の前倒し、既定カバー文言の圧縮と大きめタイポグラフィ戦略は中国語の事実源で収口済みです。次段階はまだ候補分析段階であり、正式な roadmap / todo には昇格していません。
- **第二十一段階の主軸**: 中国語の事実源では、UI 実環境テストフロー治理、スクリプト专项最適化と実行入口治理、タグ翻訳の前置化と本文翻訳の分離、既定カバー生成文言の圧縮と大きな文字サイズ戦略の 4 本を現行主線として扱います。
...
- **要約ページとしての範囲**: 本ページは第十八から第二十段階までのアーカイブ要約と第二十一段階の方向要約のみを保持し、詳細な受け入れ条件とタスク分解は中国語の `roadmap.md` と `todo.md` を優先します。
