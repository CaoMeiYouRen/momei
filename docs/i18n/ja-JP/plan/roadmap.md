...
- **`ja-JP` 正式整列は進行中**: `zh-CN` を基準にした parity スクリプト `scripts/i18n/check-locale-parity.mjs` と `pnpm i18n:check-sync` を導入し、管理画面 shell、設定ソースバッジ、`admin-ad`、`admin-external-links`、`admin-friend-links`、`admin-ai`、`admin-marketing` などの段階的補完を進めています。
