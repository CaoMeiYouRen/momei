...
- 마이그레이션 기간에는 기존 `docs/<locale>/` 경로를 잠시 유지할 수 있지만, 하나의 공개 라우트에는 하나의 실제 소스만 남겨야 합니다.
...
- 페이지를 `docs/<locale>/`에서 `docs/i18n/<locale>/`로 옮길 때는 기존 복제본을 같은 변경에서 제거하고 VitePress rewrites와 editLink도 함께 맞춰야 합니다.
