...
- **25단계는 이제 정식 계획으로 승격되었습니다**: 중국어 `roadmap.md` 와 `todo.md` 에서는 배포 및 초기화 경험, 에디터와 본문 렌더링의 2차 수렴, 빌드 속도와 번들 성능 심화, Cloudflare 런타임 적합성 연구, 그리고 AI 초기화 / 설정 도우미 평가라는 5개 방향을 다음 단계의 정식 실행면으로 올렸습니다.
- **요약 범위 안내**: 이 페이지는 최근 감사가 완료된 단계의 요약과 현재 정식으로 상향된 단계의 간단한 상태를 함께 제공합니다. 상세 수용 기준과 작업 분해는 중국어 원문 `roadmap.md` 및 `todo.md`를 기준으로 합니다.
