...
- **32단계는 이제 공식 계획 단계입니다**: 다국어 콘텐츠 자산화 강화 패키지의 통합 유입 페이지가 이번 단계의 유일한 신규 기능으로 승격되었고, coverage / 테스트 유효성 향상, 중복 코드 / 순수 함수 재사용, 단일 규칙 ESLint / 타입 부채 슬라이스, 그리고 Postgres 요청 입구 또는 공개 핫리드 체인 한 조각 거버넌스가 함께 상수로 묶였습니다. 코드 주석 거버넌스는 backlog 의 예비 슬라이스로만 유지되며 이번 단계의 정식 범위에는 포함되지 않습니다.
