...
- **35단계는 이제 정식 계획 단계로 전환되었습니다**: 이번 단계는 새 기능을 추가하지 않고 “신규 기능 0개 + 최적화 5개” 조합으로 전환하며, 중국어 사실원에서는 AI task 계량 구간과 프런트 직출 TTS 회귀 방지, Postgres 핫 리드 / DB 기동 경계 거버넌스, 다음 ESLint / 타입 부채 슬라이스, 중복 코드·흩어진 타입·순수 함수 / 유틸리티 함수를 포함한 구조 재사용 거버넌스, 기존 코드 주석 거버넌스 후보군 A 를 현재 실행 면으로 다룹니다.
