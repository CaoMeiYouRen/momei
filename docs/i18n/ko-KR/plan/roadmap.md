...
- **20단계가 정식으로 시작되었습니다**: 다음 공식 단계는 품질 게이트, release 자동화, 보안 알림 폐쇄, 중복 코드 검출 자동화를 우선하며, 새로운 배포 기능이나 상업화 확장은 그 이후로 미룹니다.
- **20단계의 현재 주축**: 중국어 사실원 기준으로 브라우저 / E2E 안정화, Release 와 Review Gate 자동화 통합, Dependabot / Code Scanning 경보 폐쇄, 중복 코드 검출 스크립트 기반선의 네 갈래가 현재 계획 주선입니다.
- **요약 범위 안내**: 이 페이지는 18단계와 19단계 아카이브 요약 및 20단계 방향 요약만 제공하며, 상세 수용 기준과 작업 분해는 중국어 원문 `roadmap.md` 및 `todo.md`를 기준으로 합니다.
