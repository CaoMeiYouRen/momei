...
- **28단계는 이제 공식 실행 단계에 들어갔습니다**: 중국어 사실원에서 “신규 기능 1개 + 최적화 4개” 조합으로 5개 주선을 열었으며, 관리자 콘텐츠 인사이트 대시보드, PostgreSQL CPU / 연결 수명 균형화, coverage `76%+` 상향, i18n 런타임 로딩 거버넌스, 에디터 Markdown / 시각 일관성 강화가 현재 실행 범위입니다.
