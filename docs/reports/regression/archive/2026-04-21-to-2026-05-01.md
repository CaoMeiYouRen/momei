...
- 本轮覆盖：`docs/design/governance/archive/hexo-repository-sync.md` 专项设计；`server/services/post-hexo-repository-sync.ts` 与 `server/api/admin/posts/[id]/hexo-repository-sync.post.ts` 的手动同步链路；`server/services/post-distribution.ts` 的统一分发摘要派生；以及 `HEXO_SYNC_*` 设置项与相关说明文档。
...
- 验证层级：`V0 + V1 + V2 + RG`。
- V0：专项设计文档、配置契约、provider 差异与回滚边界已在 `docs/design/governance/archive/hexo-repository-sync.md` 固化。
...
- 文档与事实源检查：`docs/design/governance/archive/hexo-repository-sync.md`、`docs/guide/features.md`、`docs/guide/variables.md`
