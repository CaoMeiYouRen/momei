...
- **布局**: 与登录 / 注册页保持同一套居中卡片结构与品牌头部。
- **交互边界**: 忘记密码页负责邮箱提交与状态反馈；重置密码页负责 token 校验、新密码输入与结果提示。
- **实现约束**: 当前详细流程以 `better-auth` 的现有路由能力与前端实际页面为准；若后续引入双因素、魔法链接或更复杂的恢复流程，再另行补充专项设计文档。
