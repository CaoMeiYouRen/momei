...
### 2.6 配置项多语言治理 (Localized Settings Governance)
- **治理对象**: 第十四阶段开始收敛“管理员可编辑且面向读者展示”的配置文案，例如 `site_title`、`site_description`、`site_keywords`、公告文案和法律文本简介。
- **模型边界**: 该类字段继续保存在 `Setting` 的轻量 KV 模型中，但 `value` 将按结构化 JSON 承载逐语言内容；`site_url`、布尔开关、密钥、限制项等仍维持全局单值，不参与 locale 回退。
- **兼容策略**: 历史单字符串旧值不会被立即打断，而是作为 `legacyValue` 兼容层参与最后一级回退，并在后台显式提示迁移状态。
- **统一消费**: 公开设置、SEO、邮件、通知等链路后续必须复用同一套 locale 解析器，而不是各自写 `zh-TW -> zh-CN` 之类的私有逻辑。
- **协议边界**: 法律协议正文继续由 `AgreementContent` 负责权威版本与译本治理，不回退到设置模型；仅“法律文本简介 / 摘要”类短文案纳入配置项多语言治理。
...
- **配置项多语言化**: 推进可翻译配置项的结构化存储、 locale 回退解析器与后台逐语言编辑能力，统一公开设置、SEO、邮件和通知的消费口径。
