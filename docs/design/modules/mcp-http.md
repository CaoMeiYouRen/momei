...
| **工具注册策略（Phase 1）** | `momei-mcp-server` 包自身导出 `createMcpHttpServer` | 避免代码复制；包自身支持双模式（stdio + HTTP）；通过 workspace 协议引用 |
| **Transport 选择** | `WebStandardStreamableHTTPServerTransport` | h3 v2 基于 Web API；h3 event 可通过 `getRequestURL`/`readRawBody` 构造 `Request` 对象
...
│  │ 共用 API Key │   │    import('momei-mcp-server') │    │
│  │ 鉴权中间件    │   │      .createMcpHttpServer()  │    │
│  └──────────────┘   │    store transport            │    │
│                      └──────────────────────────────┘    │
│                                                          │
│  ┌──────────────────────────────────────────────────┐    │
│  │    momei-mcp-server (packages/mcp-server)         │    │
│  │                                                    │    │
│  │  createMcpHttpServer() {                           │    │
...
```
┌─ Nitro Plugin (server/plugins/mcp-http.ts) ──────────┐
│  条件守卫 → 动态导入 momei-mcp-server                  │
│  → createMcpHttpServer() → 暴露 transport → 注册 close │
└──────────────────────┬────────────────────────────────┘
                       │
┌──────────────────────▼────────────────────────────────┐
...
└──────────────────────┬────────────────────────────────┘
                       │
┌──────────────────────▼────────────────────────────────┐
│  Route Handler (server/api/mcp/index.ts)               │
│                                                        │
│  defineEventHandler(async (event) => {                  │
│    validateApiKeyRequest(event)  ← DB-level API Key    │
│    const req = new Request(url, { method, headers, body })│
│    const res = await transport.handleRequest(req)       │
│    setResponseStatus + setHeader + return res.body      │
│  })                                                     │
└────────────────────────────────────────────────────────┘
```
...
实际实现使用 **`WebStandardStreamableHTTPServerTransport`**（Web Standard 版本），而非 Node.js 原生版本。原因：

- h3 v2 基于 Web API，h3 `event` 不直接暴露 Node.js `IncomingMessage`
- 通过 `getRequestURL(event)`、`event.method`、`getHeaders(event)`、`readRawBody(event)` 构造 `new Request()`，然后委托给 transport
- 无需 `@hono/node-server` 桥接层
...
const webRequest = new Request(url, { method, headers: reqHeaders, body })
...
Phase 1 采用 **工具注册逻辑内置在 `momei-mcp-server` 包中**的策略。`momei-mcp-server` 导出 `createMcpHttpServer()` 高层函数，Nitro Plugin 直接调用：
...
| 步骤 | 内容 | 状态 |
|------|------|------|
| 1.1 | 新增环境变量 `MOMEI_ENABLE_MCP_HTTP` 并注册到配置文档 | ✅ |
| 1.2 | 创建 `server/plugins/mcp-http.ts`：条件守卫 + 动态导入 | ✅ |
| 1.3 | 集成 `WebStandardStreamableHTTPServerTransport` + 路由分发 | ✅ |
| 1.4 | 复用 API Key 鉴权中间件 + CORS 适配 | ✅ |
| 1.5 | `momei-mcp-server` 包导出 `createMcpHttpServer` + `SERVER_NAME`/`SERVER_VERSION` | ✅ |
| 1.6 | 新增 `@modelcontextprotocol/sdk` 根依赖 + `pnpm install` | ✅ |
| 1.7 | Nitro inline 配置确保生产构建包含 momei-mcp-server | ✅ |
| 1.8 | Plugin 测试 + Route 测试（7 条用例） | ✅ |
...
| 2.2 | 速率限制集成 + 危险操作验证 | 0.5h |
| 2.3 | E2E 测试（实际 MCP 客户端调用） | 1h |
| | **小计** | **3h** |
...
| 步骤 | 内容 | 估时 |
|------|------|------|
| 3.1 | 无状态模式支持（Serverless 友好） | 1h |
| 3.2 | 健康检查端点 `GET /api/mcp/health` | 0.5h |
| 3.3 | 速率限制集成 | 0.5h |
...
> **设计状态**: Phase 1 已完成
> **关联包**: `packages/mcp-server` + `server/plugins/mcp-http.ts` + `server/api/mcp/index.ts`
> **依赖**: `@modelcontextprotocol/sdk@^1.29.0`（根依赖）+ `momei-mcp-server@workspace:*`
> **测试**: `server/plugins/mcp-http.test.ts` + `server/api/mcp/index.test.ts`（7 条用例）
