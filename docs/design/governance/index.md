...
- **[Windows 本地 Dev / Build 性能治理](./windows-dev-build-performance-governance)**: 本地 Windows 生命周期治理文档，聚焦首请求阻塞、构建尾耗时、量化脚本与后续切片边界。
...
- **[Harness Engineering 方法论引入方案](./harness-engineering-adoption)**: 基于《从 Vibe Coding 到 Harness Engineering》的五层 harness 体系分析，优先引入认知层（推理模式切换）和交接层（Session 级任务协议），并补充边界层 Hooks / 插件的跨平台调研与 phased 接入意见。
