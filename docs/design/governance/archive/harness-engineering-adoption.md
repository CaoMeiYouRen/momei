...
**时间表**: 2026-05-17 ~ 约 1 - 2 天（Phase A）；2026-06-05 Phase B 跟踪启动
**目标**: 基于《从 Vibe Coding 到 Harness Engineering》一文的五层 harness 体系分析，将与本项目现有治理骨架相容、且能跨平台（Claude Code / GitHub Copilot / OpenCode）生效的高优先级层引入。Phase A 已完成仓库侧实现（推理模式切换、Session 级任务连续性、四工具检索哲学、边界层 hooks 基础接入）；Phase B 扩展 PostToolUse 质量校验与 Stop 门禁。
...
2. **GitHub Copilot**：已在 `.github/hooks/momei.json` 绑定 `sessionStart`、`postToolUse`、`preCompact`、`sessionEnd` 四类事件，统一调用同一脚本；Windows 端显式提供 `powershell` 命令，避免只依赖通用 `command` fallback。后续基于专用 hooks 日志的排查进一步确认，`postToolUse` warning 的真实根因不是 PowerShell 本身，而是 hook runner 侧的 stdin EOF 语义不稳定，旧版脚本在 `readStdin()` 中无限等待 EOF，最终被 Copilot 在 15 秒后超时终止。
...
> **状态更新（2026-06-08）**：出于性能与主机稳定性考虑，`postToolUse` / `PostToolUse` / `tool.execute.after` 的写后 hooks 已从 Claude / Copilot / OpenCode 三端移除。直接原因包括：1）工具执行耗时增加；2）短时间内生成多个 node.js 进程导致 CPU/内存占用冲高。当前保留的仍是 `session-start`、`pre-compact`、`session-end`、`pre-stop-check` 等轻量事件。
...
- 已补充 Copilot hooks 的 `bash` / `powershell` 平台命令入口，并在 Windows PowerShell 下手动执行 `session-start` / `post-tool-use`，确认脚本退出码为 `0` 且未产生额外 stderr。
- 已通过专用日志 `GitHub Copilot Chat Hooks.log` 复核真实故障链路：旧版 `postToolUse` 会在 `Hook command timed out after 15s` 后以 `Completed (NonBlockingError)` 结束，并伴随 `Exception ignored on flushing sys.stdout: OSError: [Errno 22] Invalid argument`；该 warning 是 hook 子进程超时后的表象，而不是仓库脚本主动输出的错误。
- 已新增“stdin 写入 payload 但故意不关闭”的本地复现验证：子进程方式执行 `scripts/ai-hooks/session-governance.mjs --platform=copilot --event=post-tool-use`，保持 stdin 打开，脚本仍可在 2 秒内自行退出且 `stdout/stderr` 为空，确认 `readStdin()` 的有界收包修复生效。
- 已补跑正常关闭 stdin 的回归验证：PowerShell 管道执行 `node scripts/ai-hooks/session-governance.mjs --platform=manual --event=session-start`，仍可正常返回 `additionalContext`，确认未破坏标准管道输入路径。
...
- **边界层 Hooks / 插件接入**：已从"搁置观察项"升级为下一轮可执行候选 → **已于 2026-06-05 正式启动 Phase B**，详见 [harness-phase-b-quality-gates.md](./harness-phase-b-quality-gates.md)。
- **证据闭环增强**：当 Review Gate 中出现 agent 未实际跑验证却声称通过的情况时，升级为 P0 blocker 列入下一阶段。
- **语义 RAG 系统**：当 `docs/` 目录数超过 200 个且当前多工具检索组合出现明显效率下降时，重新评估。
