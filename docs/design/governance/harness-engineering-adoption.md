...
**目标**: 基于《从 Vibe Coding 到 Harness Engineering》一文的五层 harness 体系分析，将与本项目现有治理骨架相容、且能跨平台（Claude Code / GitHub Copilot / OpenCode）生效的高优先级层引入，优先补齐"认知层（推理模式切换）"、"交接层（Session 级任务连续性）"，并补充边界层 Hooks / 插件机制的跨平台调研与接入意见。
...
共享治理逻辑：
  scripts/ai-hooks/
    session-briefing.mjs
    session-flush.mjs
    track-tool-change.mjs
    verify-after-write.mjs

平台薄适配：
  .claude/settings.json
  .github/hooks/*.json
  .opencode/plugins/session-governance.ts
```
...
若下一轮正式实施，建议新增或修改以下文件：

| 文件 | 操作 | 说明 |
|:---|:---|:---|
| `.github/hooks/momei.json` | 修改 | 定义 Copilot 仓库级 hooks 绑定 |
| `.claude/settings.json` | 修改或新增本地模板 | 定义 Claude Code 仓库级 hooks |
| `.opencode/plugins/session-governance.ts` | 新增 | OpenCode 的等效插件适配层 |
| `scripts/ai-hooks/session-briefing.mjs` | 新增 | 统一读取 `.session/` 并生成 briefing |
| `scripts/ai-hooks/session-flush.mjs` | 新增 | 统一更新 `current-task.yaml` 与 `runtime-state.json` |
| `scripts/ai-hooks/track-tool-change.mjs` | 新增 | 统一记录编辑文件与验证状态 |
| `scripts/ai-hooks/verify-after-write.mjs` | 新增 | 统一做写后最小验证或生成提醒 |
