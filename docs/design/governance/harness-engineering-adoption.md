...
2. **GitHub Copilot**：已在 `.github/hooks/momei.json` 绑定 `sessionStart`、`postToolUse`、`preCompact`、`sessionEnd` 四类事件，统一调用同一脚本；Windows 端显式提供 `powershell` 命令，避免只依赖通用 `command` fallback。
