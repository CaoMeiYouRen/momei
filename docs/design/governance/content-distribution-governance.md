...
### 2.3 实现校准（截至 2026-03-16）

- 发布链路已不再自动触发 Memos 同步，历史兼容字段 `syncToMemos` 仅保留为弃用输入，不再驱动真实分发行为。
- Memos 远端链接回写已统一为真实公开 permalink；若实例地址配置为 `/api/v1` 结尾，会在回写前自动归一化为公开站点基址。
- WechatSync 完成回写已支持账户级结果、失败分类与人工终止；时间线会记录首个成功草稿链接，渠道状态则按最终成功/失败结果收口。

## 3. 当前问题

### 3.1 发布副作用耦合

- 设计阶段的历史状态中，`server/services/post-publish.ts` 会在发布时根据 `syncToMemos` 立即同步到 Memos。
- 该行为已经在当前实现中解除；相关动机仍保留在本设计文档中，用于说明为什么需要独立的手动分发状态机与审计面板。
...
- 回写远端链接时统一使用公开 permalink，而不是内部 API 路径或错误的实例 URL。

### 8.2 WechatSync

- 服务端负责创建尝试记录、状态机流转、人工终止与最终结果入库。
- 前端负责检测插件、选择账户、发起实际 `window.$syncer.addTask()` 调用，并在完成后把账户级结果回写到服务端。
- 结果回写会按账户级错误摘要归一失败原因，并将草稿链接保留在时间线详情中，便于后台审计与人工复查。
