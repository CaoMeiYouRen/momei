...
- 配置与入口：在后台系统设置中维护仓库参数，并在文章分发面板统一触发与查看最近状态。
...
- 后台“系统设置 -> 第三方集成”提供 Hexo 同步配置项。
- 后台文章分发面板提供统一入口与最近状态展示。
...
- 管理后台现有分发弹窗可复用统一状态展示与手动触发交互。

### 3.2 缺口

- 目前没有任何 GitHub / Gitee 仓库写入实现。
- 现有分发摘要原本只覆盖 `memos` 与 `wechatsync`，需要补一层派生状态才能把 Hexo 能力接入统一入口。
...
其中 `HEXO_SYNC_ACCESS_TOKEN` 仍视为敏感配置，但已接入系统设置中心并按密码字段脱敏展示；若部署层提供同名环境变量，则继续按 ENV 锁定只读，不允许后台覆盖。

当前实现中，`HEXO_SYNC_*` 已进入通用系统设置页的第三方集成分组，管理员可在后台维护仓库提供商、Owner、Repo、分支、文章目录与访问令牌。
...
同时复用现有 `GET /api/admin/posts/:id/distribution` 摘要接口，为文章分发面板补充 `hexoRepositorySync` 派生状态，统一展示最近一次同步结果、目标文件路径与远端链接。
...
- 单元测试覆盖绝对媒体 URL 改写。
- 单元测试覆盖鉴权失败分类与元数据写回。
- 单元测试覆盖分发摘要中的 `hexoRepositorySync` 派生状态。
...
1. 支持批量同步和定时同步。
2. 支持媒体镜像到仓库或对象存储中转目录。
3. 评估 SSH / Git push 路线，用于多文件事务性提交。
4. 引入冲突检测与远端改动保护策略。

当前结论：后台系统设置项与文章分发面板统一入口已经落地，原“后续增量方向”中的第一条已完成。
