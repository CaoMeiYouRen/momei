# OpenCode + GitHub Codespaces 提供商收敛集成方案（opencode-go / deepseek / xiaomi）

## 1. 概述 (Overview)

本文档用于把“Hermes + OpenCode + GitHub Codespaces 集成方案”落地为可执行的仓库改动清单，并明确当前模型提供商收敛策略：

- 保留 `opencode-go`、`deepseek`、`xiaomi` 三类模型提供商。
...
| [scripts/setup/switch-models.mjs](../../../scripts/setup/switch-models.mjs) | Update | 增加 provider allowlist 校验（仅允许 opencode-go / deepseek / xiaomi）；保留 `--list`、`--current`。 |
...
3. 当 `opencode-go` 不可用时，可快速切换到 `deepseek` 或 `xiaomi` 预设。
...
```bash
pnpm node scripts/setup/switch-models.mjs xiaomi
```
