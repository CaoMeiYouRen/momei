...
2. 路线图仅提供摘要页，按阶段结论同步，不要求逐段全量跟进。
3. 其他 Guide / Standards / Design 页面默认后续补齐，不得因为尚未翻译而阻塞 `ui-ready` 准入。
...
3. `ja-JP` 首轮仅要求 README、文档首页、快速开始、翻译治理、路线图摘要与必要贡献入口；未覆盖页面必须显式回链中文原文。
