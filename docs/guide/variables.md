...
| `HEXO_SYNC_ACCESS_TOKEN` | `hexo_sync_access_token` | 3 | password | 仓库写入令牌，后台仅脱敏展示；若通过环境变量注入则保持锁定 |
...
补充：`HEXO_SYNC_*` 已接入后台“系统设置 -> 第三方集成”面板。`HEXO_SYNC_ACCESS_TOKEN` 在后台按密码字段脱敏展示；若部署层同时提供同名环境变量，则该字段会被系统判定为 ENV 锁定，后台只读。
