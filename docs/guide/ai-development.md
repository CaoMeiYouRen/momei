...
-   **复杂问题先搜索再动手**：当遇到不熟悉的库/框架、跨平台差异、或根因不明确的问题时，应要求 AI 先通过搜索工具调研外部信息（官方文档、issue tracker、社区最佳实践），而不是仅凭训练记忆直接给方案。尤其当同一问题修复失败超过 1 次，必须切换为搜索优先模式。详细规则见 [AI 协作规范 - 搜索优先](../standards/ai-collaboration.md#14-信息获取与搜索优先-search-first)。
