...
> 当前执行策略: 已完成第一轮统一外部自动化契约设计，并落地 API Key 驱动的外部 AI 自动化接口、CLI 命令面与 MCP 工具面。当前范围聚焦标题建议、标签推荐、整篇翻译、封面图生成、音频生成、任务查询与发布编排；后续继续补齐更细粒度测试、异常恢复与文档同步。专项设计见 [CLI / MCP 自动化能力扩展](../design/modules/cli-mcp-automation.md)。
