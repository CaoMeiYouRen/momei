...
- [ ] **通知历史与投递审计**
	...
	- 当前拆解:
		- [ ] 用户历史列表：在现有通知中心之外补齐历史通知查询接口、分页列表与已读状态回填。
		- [ ] 管理员投递审计：为通知发送结果沉淀 `channel`、`status`、`recipient`、`sentAt` 等筛选字段，并提供后台列表页。
		- [ ] 定向回归：补齐通知列表、投递日志与筛选条件的契约 / 交互测试。
...
- [ ] **全文翻译工作流重构** (P0, 进行中)
	...
	- 当前拆解:
		...
		- [ ] 字段覆盖清理：翻译前按字段范围显式清空目标语言版本的旧标签与其他可重建字段，确保重跑翻译时不残留旧值。
		- [ ] 媒体同步策略：为封面、播客音频等附件建立可选同步策略，默认封面跟随翻译版本同步、音频默认不同步，并允许用户覆盖默认值。
...
- [ ] **分类 / 标签翻译关联治理** (P1)
	...
	- 当前拆解:
		...
		- [ ] 分类显示语义收口：区分“界面语言展示”与“文章语言翻译实体”两套语义，修正分类翻译保存后回退为界面语言文本的问题。
...
- [ ] **初始化引导首配清单重构**
	...
	- 当前拆解:
		- [ ] 首配清单盘点：盘点 installation wizard、demo banner、onboarding 与 settings 页面现有字段，按“必须 / 推荐 / 可后置”重排。
		- [ ] 触发时机收口：重新定义首次进入安装向导、首次进入后台与首次进入编辑器时的提示节奏。
		- [ ] 文案与回归：补齐安装向导 / onboarding 的多语言文案与定向回归。
- [ ] **文章管理页媒体预览增强**
	...
	- 当前拆解:
		- [ ] 封面预览：在文章管理列表加入封面缩略图或预览触发器。
		- [ ] 播客快速预览：为播客列补齐音频状态、播放器或外链预览入口。
		- [ ] 定向回归：补齐文章管理页媒体列渲染与预览交互测试。
- [ ] **反馈入口与闭环**
