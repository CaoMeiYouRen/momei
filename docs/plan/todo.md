...
- [ ] **通知矩阵与站内信体系深化 (Notification Matrix & In-app Refinement)** (P1)
    - [ ] **多维矩阵实体**: 实现 `AdminNotificationConfig` 和 `UserNotificationPreference` 实体。
    - [ ] **统一分发服务**: 重构 `notification.ts` 服务，支持基于矩阵的策略分发逻辑。
    - [ ] **管理员矩阵 UI**: 后台增加“通知设置矩阵”，支持按事件 x 渠道进行分级控制（Better-Auth 事件设为只读）。
    - [ ] **推送统计与预警**:
        - [ ] 实现 `NotificationStatistics` 统计逻辑。
        - [ ] 后台增加“成本看板”，监控邮件发送频率与异常消耗。
    - [ ] **用户通知中心**:
        - [ ] 新增 `pages/notifications.vue` 历史记录页面。
        - [ ] 实现通知的“已读/删除/清理”逻辑。
        - [ ] 优化个人设置页，同步展示分级设置矩阵。
...
### 11. 质量保障与自我验证 (Quality Assurance & Self-Verification) (P1)
- [ ] **引入 Playwright 端到端测试框架**
    - 验收: 完成 `@playwright/test` 的安装与集成。
    - 验收: 编写针对首页的测试用例，涵盖正常加载、暗色模式切换、国际化语言切换等核心交互。
    - 验收: 实现 E2E 测试与开发环境的自动关联运行。
- [ ] **自我验证机制集成**
    - 验收: 更新测试规范文档，明确 E2E 测试编写标准。
    - 验收: (可选) 探索集成自动化验证脚本，实现开发完成后的自动回归。
