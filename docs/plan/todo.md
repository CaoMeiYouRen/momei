...
- [ ] **系统设置与智能混合模式增强 (System Settings & Hybrid Mode) (P1)**
    - [ ] 验收: 实现“智能混合模式”逻辑，确保环境变量 (ENV) 优先级高于数据库且在 UI 中提示锁定。
    - [ ] 验收: 扩展设置项：增加社交登录 (OAuth)、验证码安全 (Captcha)、系统限额 (Limits) 及品牌增强字段。
    - [ ] 验收: 优化安装向导：支持安装时自动从 ENV 载入现有配置到数据库。
    - [ ] 验收: 完善 Admin UI：新增“认证”、“安全”、“限额”标签页，并增加 ENV 锁定视觉反馈。
