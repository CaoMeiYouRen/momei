...
- [ ] **库存对齐、Skills 复用与冗余裁剪**
	...
	- 验收: 要求 agent 优先复用既有 skills 与规范文档，不再把相同的执行门禁、质量要求与流程说明复制到多个 agent 文件中。
- [ ] **Rules 层最小化与平台适配**
...
- [ ] **AI 初始化 / 问答助手评估与工程复用治理**
	...
	- 验收: 为“公共 utils 抽取、业务逻辑复用、组件模板复用、SCSS 片段复用、大文件拆解”补齐专门的 agent 或 skill 治理范围，并明确这类事项默认属于持续优化与质量建设，不自动并入功能迭代。
- [ ] **配置体检与最小验证闭环**
...
- [ ] **周期性回归清单与漂移治理**
	- 验收: 整理需要定期回归的事项，至少覆盖代码优化与复用收敛、ESLint warning / 类型债治理、`database/*/init.sql` 与实体/设计文档同步、README / 部署 / 翻译文档同步、测试用例补齐与性能基线漂移。
	- 验收: 明确这些事项默认属于独立治理任务，不自动膨胀进普通功能需求；仅在阻塞交付或构成功能回归时允许插队。
	- 验收: 补齐回归任务模板，至少覆盖回归范围、触发条件、执行频率与输出格式 4 个字段。
