...
- [x] **i18n 运行时继续扩面 (P1)**
	...
	- 最新进度: 2026-05-04 已修正 `auth-card` 真实 key 传递契约，移除 `taxonomy-post-page` 的 RSS `aria-label` 硬编码，并将 `pages/forgot-password.test.ts`、`pages/reset-password.test.ts`、`tests/pages/taxonomy-rss-discovery.test.ts` 纳入 `i18n:verify:runtime`；`pnpm i18n:audit:missing` 已确认 `total: 0`，`pnpm exec vitest run --config vitest.i18n.config.ts` 通过，因此条目正式关闭。
