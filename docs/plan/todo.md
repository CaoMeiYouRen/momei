...
当前进行中事项：
- `ja-JP` 正式对齐治理：已开始建立 zh-CN 基准 parity 校验脚本，完成 ja-JP admin 高频字段首轮缺口，并继续补齐第二批 admin 模块与日文快速开始文档同步。
...
- [ ] **将 `ja-JP` 从 `ui-ready` 升格为正式对齐维护语种**
	...
	- 进展: 已新增 `scripts/i18n/check-locale-parity.mjs` 与 `pnpm i18n:check-sync`，统一以 `zh-CN` 为基准输出各 locale / module 的缺失字段与多余字段；已完成 `ja-JP` 后台壳层、设置来源徽标与 admin 高频标题的首轮补齐，并继续补齐 `admin-external-links`、`admin-ad`、`admin-friend-links` 第二批后台模块与 `docs/i18n/ja-JP/guide/quick-start.md` 同步。
