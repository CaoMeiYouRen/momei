...
- 进行中（2026-05-01）：第三十二阶段 `Postgres 查询、CPU 与连接生命周期平衡治理 (P0)` 当前选择“公开热点读链路”切片，先推进 `/api/search` 的匿名短 TTL 缓存与定向断言；`pg_stat_statements` / 等价 live sample 证据仍待补齐，因此本主线继续保持进行中。
...
- [ ] **Postgres 查询、CPU 与连接生命周期平衡治理 (P0)**
	...
	- 推进记录（2026-05-01）: 带会话请求继续旁路共享缓存，避免订阅可见性差异污染匿名缓存；当前仍待补 `pg_stat_statements` 或等价 live sample，再决定是否可关闭本主线。
