...
1. [ ] **后台内容统计与热点分析看板 (P1)**
	...
	- 进度说明: 已将 `/admin` 改为第一版内容洞察首页，新增 `GET /api/admin/content-insights`、多语言文案、代表版本去重口径与定向 Vitest 验证，并通过定向 typecheck 与 i18n lint；当前阅读 / 评论仍是窗口内容的累计表现，待事件级历史能力补齐后再关闭该项。
