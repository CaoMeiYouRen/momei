...
-   [x] **阅读体验优化 (基础)**
    ...
    -   验收: 文章底部显示版权声明组件。
-   [ ] **版权声明系统优化 (进行中)**
    -   分析: 引入预定义的版权协议 ID (CC 4.0 家族及 "所有权利保留")。
    -   分析: 默认值逻辑：从原先的 CC 协议改为“所有权利保留 (All Rights Reserved)”，保护创作者版权。
    -   分析: UI 增强：管理后台提供协议下拉选择。
    -   分析: 国际化：支持协议名称与链接的多语言映射 (zh-CN/en-US)。
-   [ ] **高级搜索**
...
### 6. 国际化与内容优化 (I18n & Content Optimization)

-   [ ] **数据库索引重构**
    -   验收: 修改 `Post`, `Category`, `Tag` 实体，将 `slug` 的唯一索引改为 `(slug, language)` 联合唯一。
    -   验收: 编写相应的数据库迁移脚本。
-   [ ] **管理后台实体语言支持**
    -   验收: 文章管理列表显示文章所属语言，并支持按语言筛选。
    -   验收: 分类与标签管理列表显示对应语言标识。
    -   验收: 用户管理列表显示用户偏好语言。
-   [ ] **智能语言切换器**
    -   验收: 在文章/分类详情页切换语言时，若存在关联的翻译版本（通过 `translationId`），则跳转到该语言对应的详情 URL，而非首页。
-   [ ] **用户个性化设置**
