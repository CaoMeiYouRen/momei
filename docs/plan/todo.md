...
- [ ] **主题系统重构**
    - 验收: 实现核心逻辑与样式的解耦，支持第三方主题动态加载。
    - 验收: 管理后台支持切换主题并保存配置。
- [ ] **个性化定制**
    - 验收: 支持管理员在后台注入自定义 CSS/SCSS 样式。
- [ ] **感官体验增强**
    - 验收: 实现哀悼模式（全站置灰）、看板娘、背景粒子等视觉增强开关。

### 2. 互动与隐私控制 (Interactions & Privacy Control)

- [ ] **精细化文章访问权限控制**
    - 验收: 实现文章可见性策略：公开、私密、密码保护、登录可见、订阅可见。
    - 验收: 支持针对非公开内容显示引导性/校验 UI。
- [ ] **原生评论系统**
    - 验收: 实现内置评论组件，支持登录用户互动、点赞。
    - 验收: 管理后台支持评论审核与黑名单管理。
- [ ] **访客投稿工作流**
    - 验收: 实现投稿入口，支持外部创作者提交草稿至待审核列表。
...
- [ ] **高性能 PV 统计优化**
    - 验收: 实现基于内存缓存的批量写入逻辑，降低高流量下数据库主表更新压力。
- [ ] **存储系统进化**
    - 验收: 实现 `LocalStorage` 适配器，并完善 Serverless 环境冲突卫兵检测逻辑。
- [ ] **文档国际化**
...
### 7. 精准推送与订阅 (Advanced Subscription)
- [ ] **维度级订阅推送**
    - 验收: 支持用户按分类/标签订阅特定邮件内容。
- [ ] **推送频率管理**
    - 验收: 实现用户的日报/周报聚合推送模式。

### 8. 商业化集成 (Monetization)
- [ ] **支付与会员体系**
