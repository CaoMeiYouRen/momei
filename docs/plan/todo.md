...
> 第十三阶段已正式立项。以下待办仅覆盖本阶段 5 项核心任务，不额外并入 backlog 中的长期规划。
...
- [ ] **多语言封面生成与回填口径统一**
	- 进展: 已开始实现目标语言 prompt 透传、封面 locale 绑定元数据与翻译流程默认禁复用源语言封面。
...
- [ ] **多语言 TTS / Podcast 资产绑定**
	- 进展: 已开始实现音频 locale / translationId / postId 绑定写回，并禁止翻译流程默认复制源语言音频。
...
- [ ] **渠道导出模板分层**
	- 验收: 为 Memos、WechatSync、B 站专栏等主要渠道区分正文、摘要、封面、标签与版权尾注的导出模板，不再沿用单一 Markdown / HTML 模板。
	- 验收: 模板分层不得破坏现有手动同步、重试、结果回写与分发审计链路。
	- 验收: 补齐主要渠道的模板输出测试，至少覆盖标题层级、图片、版权尾注与纯文本摘要 4 类内容差异。
- [ ] **标签清洗与渠道适配规则统一**
	- 验收: 为 `#标签` 导出建立统一的清洗、去重、长度限制与非法字符处理规则，并支持渠道差异化适配。
