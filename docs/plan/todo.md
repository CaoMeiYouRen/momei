...
- [ ] **流量增长与变现 (Growth & Monetization)**
    - [ ] 验收: **多维度显隐控制**: 支持按当前页面语言 (zh-CN/en-US) 动态决定外链/打赏码的展示与隐藏。
    - [ ] 验收: **社交链接与打赏集成**: 实现基于“个人配置+全局覆盖”的社交引流与赞助系统。
    - [ ] 验收: **多平台支持**: 覆盖微信、支付宝、爱发电等国内平台及 GitHub, Patreon, PayPal 等国际平台。
    - [ ] 验收: **自定义扩展**: 提供 1-2 个自定义位置供用户配置特定图片/链接。
    - [ ] 验收: **位置适配**: 在文章详情页末尾（正文后、版权声明前）增加显著的社交引导与打赏入口。
    - [ ] 验收: **安全校验**: 对自定义 URL 加载进行白名单与协议校验。
