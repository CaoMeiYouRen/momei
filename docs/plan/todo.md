...
- [ ] **复用抽象与质量门禁**
	...
	- 当前拆解:
		...
		- [x] 页面拆分收口：抽离 `use-installation-wizard.ts`、`use-post-editor-page.ts` 与 `use-post-editor-translation.ts`，将 `pages/installation.vue`、`pages/admin/posts/[id].vue` 收敛为装配层，消除本轮相关大文件告警。
