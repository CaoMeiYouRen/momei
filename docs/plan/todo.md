...
- [ ] **测试覆盖率与有效性治理 (P0)**
	...
	- 推进记录（2026-05-02）: 本轮继续沿“公开页 runtime / auth 配置退化 / 认证页 raw key 暴露”主线补第二批高风险断言。`lib/auth-client.test.ts` 新增 URL / Request session 输入、broadcast false、cache bust 失败告警、non-session / non-GET 直通、failed response 不缓存等退化测试，`lib/auth.ts` 新增无请求上下文 locale 回退、endpoint locale drift 与 captcha 动态配置分支测试；公开页与认证页方面则把 `pages/archives/index.test.ts`、`pages/posts/index.test.ts`、`pages/forgot-password.test.ts`、`pages/reset-password.test.ts`、`pages/register.test.ts` 补成真实文案装配、防 raw key 回退、成功 / 失败 / 异常路径与跳转断言。
	- 推进记录（2026-05-02）: 定向 coverage 已确认 `lib/auth-client.ts` 达到 statements `96.7%` / lines `96.7%`、`lib/auth.ts` 达到 statements `95.7%` / lines `100%`、`pages/archives/index.vue` 达到 statements `93.1%` / lines `93.1%`、`pages/posts/index.vue` 达到 statements `91.3%` / lines `91.3%`、`pages/forgot-password.vue` 达到 statements `100%` / lines `100%`（branches `95.45%`）、`pages/reset-password.vue` 达到 statements `97.72%` / lines `97.72%`、`pages/register.vue` 达到 statements `100%` / lines `100%`（branches `90.62%`）。本次复核额外确认 `pages/register.test.ts` 的 targeted coverage rerun 已不再出现此前的 i18n 初始化异常。距离 `80%+` 仍有明显差距，下一批高 ROI 候选继续优先保持在 `pages/login.vue`、其余认证流 / 公开页 raw key 暴露，以及共享组件与热点公开读链路的失败路径。
	- 推进记录（2026-05-02）: 已继续上收 `pages/login.vue` 这条高 ROI 认证链路，将 [pages/login.test.ts] 的结构存在断言重写为真实文案装配、防 raw key 回退、社交登录 callbackURL 归一化、demo 账号预填、email 登录成功 / 逻辑失败 / 抛异常恢复，以及验证码请求头与 rememberMe 断言；定向 Vitest `8` 条断言已全部通过，单文件 coverage rerun 确认 `pages/login.vue` 达到 statements `100%` / branches `84.44%` / functions `100%` / lines `100%`，且 targeted coverage rerun 未再出现 locale runtime loader 初始化异常。下一批高 ROI 候选顺延为其余认证流边角分支、共享组件 raw key 暴露与热点公开读链路失败路径。
