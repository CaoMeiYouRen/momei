...
当前进行中事项：
- `ja-JP` 正式对齐治理：已开始建立 zh-CN 基准 parity 校验脚本，并补齐 ja-JP admin 高频字段首轮缺口。
...
- [ ] **将 `ja-JP` 从 `ui-ready` 升格为正式对齐维护语种**
	...
	- 进展: 已新增 `scripts/i18n/check-locale-parity.mjs` 与 `pnpm i18n:check-sync`，统一以 `zh-CN` 为基准输出各 locale / module 的缺失字段与多余字段；同时已完成 `ja-JP` 后台壳层、设置来源徽标与 admin 高频标题的首轮补齐。
