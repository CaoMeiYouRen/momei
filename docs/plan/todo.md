...
- 测试覆盖率与有效性治理：正在补齐中间件异常/缓存边界/编辑器语音回退分支，并等待全量 coverage 基线收口。
...
3. [x] **国际化缺失字段治理与质量门收紧 (P0)**
	- 验收: 先优先清理各语言 locale 模块中的缺失字段，避免 raw key 直接泄漏到前端或后台 UI。
	- 验收: 明确区分“缺失字段”和“未使用字段”，当前阶段只把缺失字段作为阻断项，未使用字段先作为观察与后续清理候选。
	- 验收: 将缺失字段审计纳入当前阶段固定回归入口，至少在 release / 周期性回归前强制执行。
	- 验收: 优先治理问题数量最高的语言与模块，逐步压降全仓缺词债，而不是平均铺开。

4. [ ] **测试覆盖率与有效性治理（目标 > 76%） (P0)**
