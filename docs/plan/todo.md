...
- [ ] **通知系统强化与模块补全 (Notification Strengthening)** (P1)
    - [ ] 验收: **营销类型区分**: 扩展 `MarketingCampaign` 类型字段，支持 `UPDATE`, `FEATURE`, `PROMOTION`, `BLOG_POST`, `MAINTENANCE`, `SERVICE`六大类。
    - [ ] 验收: **博客推送联动 (二次确认机制)**: 
        - 文章发布确认框增加“推送选项”（不推送/存为草稿/立即发送）。
        - 实现首次发布时的逻辑分支，确保不同步误发。
        - 文章管理页增加“再次推送到邮件”操作，支持预设内容。
    - [ ] 验收: **管理员站务接收**: 后台增加管理员通知配置页，涵盖新用户、新评论、API 错误及系统警报。
- [ ] **邮件模板与队列优化 (Templates & Queue)** (P2)
    - [ ] 验收: **卡片式 HTML 模板引擎**: 实现基于 `table` 布局的 HTML 模板，支持封面图卡片展示。
    - [ ] 验收: **发送前测试**: 增加“发送预览邮件到管理员邮箱”的功能，确保渲染正确。
    - [ ] 验收: **变量替换引擎**: 实现支持多语言（$t）和自定义变量（{{articleTitle}}）的替换逻辑。
