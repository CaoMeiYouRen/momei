...
- 后台内容统计与热点分析看板：已落第一版“窗口内容累计表现”看板，待补事件级阅读 / 评论历史后再闭合真实趋势验收。
- 后台内容统计与热点分析看板：真实阅读趋势设计已收敛为“小时级稀疏聚合 + 查询时按时区折叠自然日”，下一步进入实体与 flush 链路设计实现。
...
1. [ ] **后台内容统计与热点分析看板 (P1)**
	- 验收: 在后台管理侧提供内容运营概览卡片，至少覆盖阅读量、评论量、发文量与最近 `7 / 30 / 90` 天窗口表现；第一轮阅读 / 评论先以窗口内容的累计表现呈现。
	...
	- 进度说明: 已将 `/admin` 改为第一版内容洞察首页，新增 `GET /api/admin/content-insights`、多语言文案、代表版本去重口径与定向 Vitest 验证；真实阅读趋势方案已收敛为“小时级稀疏聚合表 + 查询时按请求时区折叠自然日”，后续按该方案补实体、flush 写入与真实趋势查询。
