...
- [ ] **文章编辑页面 UI/UX 优化 (Article Editor UI/UX Optimization)** (P1) [in-progress]
    - [ ] 验收: **顶部工具栏聚合**: 将 AI 建议、翻译与格式化按钮进行视觉聚合，提升空间利用率。
    - [ ] 验收: **发布状态重定位**: 将文章发布状态标签移至“发布”按钮旁，与操作逻辑对齐。
    - [ ] 验收: **语言切换器优化**: 重构顶部语言切换逻辑，使其更符合直觉。
    - [ ] 验收: **“同步”图标纠偏**: 将微信/多平台同步按钮图标更改为 `pi-sync`，明确“同步”而非“分享”语义。
    - [ ] 验收: **设置侧边栏交互升级**: 
        - [x] 将 Drawer 改为非模态 Push 模式（不产生遮罩）。
        - [x] 实现内容区随侧边栏开关的平滑位移动画。
        - [x] 新建文章时侧边栏默认展开。
- [x] **修复类型检查错误**
