...
- [ ] **本地文件存储支持 (Local Storage)**
    - 验收: 实现 `LocalStorage` 适配器，支持文件保存至本地指定目录。
    - 验收: 实现环境卫兵逻辑，在 Vercel/Cloudflare 等 Serverless 环境下禁用此功能并报错提示。
    - 验收: 实现生产环境下对上传目录的静态文件访问服务方案。
    - 验收: 支持通过 `STORAGE_TYPE=local` 进行切换。
- [ ] **阅读量 (PV) 统计性能优化**
...
### 7. 播客支持 (Podcast Support)

- [ ] **播客内容管理 (Podcast Management)**
    - 验收: 实现播客音轨上传与托管支持。
    - 验收: 实现播客节目 (Episodes) 的 CRUD 管理，支持 RSS (iTunes/Podcast 规范) 订阅源生成。
    - 验收: 实现文章与播客音轨的关联展示（类似音频文章）。
- [ ] **多媒体播放体验 (Multimedia Experience)**
    - 验收: 实现全站悬浮沉浸式音频播放器，支持断点续播。
    - 验收: 集成 AI 语音合成 (TTS)，支持将博客文章一键转换为播客音频。

8.  **AI 视觉增强 (AI Visual Enhancement) (Phase 3)**

- [ ] **AI 封面图生成 (AI Cover Images)**
    - 验收: 集成 AI 图像生成服务 (如 DALL-E, Stability AI)。
    - 验收: 在文章编辑页提供“生成封面”入口。
    - 验收: 支持基于文章标题和摘要自动提取关键词并生成描述词 (Prompt)。
    - 验收: 提供多种视觉风格预设。
    - 验收: 生成的图片可自动绑定到文章封面字段并保存到存储服务。

9.  **专业翻译集成 (Phase 3)**

- [ ] **DeepL 集成**
    - 验收: 集成 DeepL API 作为高稳定性、高准确性的翻译引擎补充。
    - 验收: 支持在管理后台切换翻译引擎 (LLM vs. DeepL)。

10. **AI 语音驱动创作 (AI Voice-Driven Creation) (Phase 3)**

- [ ] **语音录入与转录 (Voice Transcription)**
    - 验收: 实现支持浏览器实时采集的语音输入组件。
    - 验收: 集成高精度转录接口 (如 OpenAI Whisper)，支持多语言自动识别。
- [ ] **语音重构生成博文 (Voice to Blog)**
    - 验收: 实现“AI 剧本模式”，将口语化的转录文本自动润色、重构为符合博客规范的 Markdown 文章。
- [ ] **多模态创作流集成 (Multi-modal Synergy)**
    - 验收: 整合“语音输入 -> 自动博文 -> 播客生成”流水线。
    - 验收: 支持在保存博文的同时，自动生成/提取对应的音频片段并关联为播客节目的草稿。
