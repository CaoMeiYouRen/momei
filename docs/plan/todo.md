...
- [x] **建立红绿测试与失败路径优先的首轮治理范围**
	...
	- 进展: 已将 `server/utils/post-access.ts` 锁定为首轮治理范围，优先覆盖订阅者状态查询失败与公共列表过滤分支；原因是它同时影响文章列表、搜索、归档、订阅源与详情权限链路，失败后果比单纯堆 coverage 更直接。
- [x] **补齐测试有效性的证据口径**
	- 验收: 在回归记录中明确记录本轮新增失败用例、修复后转绿结果、未覆盖边界与下一轮优先补齐范围。
	- 验收: 不得只把该主线落成“新增若干测试文件”，必须体现对真实缺陷约束力的提升。
	- 进展: 首轮证据已写入 [回归任务记录](./regression-log.md)，包含新增失败路径用例、定向 Vitest 与 ESLint 结果，以及下一轮待补边界。
	- 结果: 已补齐内容访问相关 API 的 `503` 异常映射测试，并将密码解锁凭据从裸 `id` 列表收敛为“签名 + 逐项过期”的 cookie 载荷，补上了过期与伪造边界测试。
...
- [x] **建立规则债务基线与首批收紧范围**
	...
	- 结果: 已保留测试与脚本豁免；迁移 / 历史遗留生产目录在本轮 `unbound-method` 扩面采样中为 `0` 命中，暂不额外编码目录级豁免，后续若出现债务再单独收口。
- [x] **完成首批规则收紧与回滚口径说明**
