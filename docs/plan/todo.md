...
- [ ] 第十五阶段 / 自动化验证分级、周期性回归与 Review Gate：统一验证矩阵、回归模板与 timeout budget 主体已落地，待按新口径继续执行后续收口与阶段归档。
...
- [ ] **周期性回归清单与漂移治理**
	- 进展: 已在 `docs/standards/planning.md` 中扩展周期性回归事项清单，覆盖代码优化与复用收敛、ESLint warning / 类型债治理、`database/*/init.sql` 与实体 / 设计文档同步、README / 部署 / 翻译文档同步、i18n 初始化字段完整性、测试与覆盖率治理、性能基线、脚本目录残留治理，以及 `max-lines` 超限文件与临时豁免清理；同时新增包含回归范围、触发条件、执行频率、timeout budget 与输出格式的统一回归任务模板，并明确这类事项默认以独立治理任务执行。
	- 验收: 整理需要定期回归的事项，至少覆盖代码优化与复用收敛、ESLint warning / 类型债治理、`database/*/init.sql` 与实体/设计文档同步、README / 部署 / 翻译文档同步、i18n 翻译大文件拆分后的初始化字段完整性、测试用例补齐、性能基线漂移，以及 `scripts/**` 中未使用长期脚本、失效入口引用、临时脚本残留与无效脚本的定期清理。
