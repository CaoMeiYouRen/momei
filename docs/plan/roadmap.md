...
1. **系统配置体验补强 (System Config UX Enhancements)**:
    - **来源解释增强**: 在后台设置页增加“智能混合模式”说明卡片，并补齐 `ENV -> DB -> Default` 的来源解释路径。
    - **结构化元信息**: 为设置接口补充 `envKey`、`defaultUsed`、`lockReason` 等元信息，降低管理员的判断成本。
    - **后台交互完善**: 为锁定项补充来源徽标与细粒度提示，形成完整的只读解释闭环。
2. **AI 成本治理与多用户配额 (AI Cost Governance & Multi-user Quotas)**:
    - **统一治理模型**: 基于 `AITask` 建立 AI/ASR/TTS 的统一用量聚合口径。
    - **分级配额策略**: 支持按角色、用户组或信任等级实施差异化额度策略。
    - **审计与告警**: 提供后台审计视图与阈值告警，为后续多用户运营与商业化预留基础。
3. **国际化语言扩展与配置模块化 (I18n Expansion & Modularization)**:
    - **多语言能力扩展**: 将当前双语言能力抽象为可配置 Locale 集合，支持后续按站点启用更多语种。
    - **配置中心化**: 收敛默认语言、回退策略与 Locale 元数据的集中配置。
    - **翻译模块拆分**: 推进翻译文件按模块拆分与按需加载，降低维护与首屏负担。
4. **多语言 SEO 深度优化 (Multilingual SEO)**:
    - **多语言元标签**: 完善 `hreflang`、canonical、Open Graph、Twitter Card 在多语言场景下的生成逻辑。
    - **结构化数据增强**: 为搜索引擎与生成式搜索补齐多语言结构化数据输出。
    - **站点地图深化**: 扩展多语言站点地图，确保语言变体能够被主流搜索引擎稳定索引。
5. **云端存储与静态资源优化 (Cloud Storage & Static Assets Optimization)**:
    - **前端直传链路**: 建立后端签名、前端直传的图片/音频/视频上传能力，减少服务端中转压力。
    - **静态资源统一管理**: 收敛 CDN 前缀、资源路径与存量链接重写策略。
    - **迁移与兼容工具**: 预留存量资源迁移与 URL 重写工具，降低存储后端切换成本。
...
### 7. 实时通知与浏览器推送 (Real-time Notifications & Browser Push)
- **浏览器推送补强**:
    - 基于现有 SSE 站内通知能力，补齐管理员站务通知与高价值异步任务完成提醒的浏览器侧投递闭环。
    - 引入 Service Worker、Push Subscription 与 VAPID 密钥管理，仅在“用户离线后仍需触达”的场景下启用 Web Push。
    - 保持分阶段策略：短中期继续以 SSE 为主，Socket.io 仅在未来出现明确的双向协作需求（如协同编辑、Presence）时再评估，避免过早引入常驻连接与额外运维成本。
...
- **工程质量提升**:
    ...
    - **解耦与模块化**: 重构核心业务模块，采用依赖注入或事件驱动等设计模式，降低模块间的耦合度，提升系统的可扩展性与可维护性。
    - **自动化质量门禁**: 增强 ESLint 与 TypeScript 检查规则，在 CI/CD 流程中引入更加严格的代码质量扫描。
