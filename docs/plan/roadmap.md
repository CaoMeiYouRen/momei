...
### 第三阶段：创新与扩展 (Innovation & Expansion) (进行中)
...
2.  **互动与隐私控制 (Interactions & Privacy Control)**:
    ...
    -   **共创模式**: 实现访客投稿流，支持外部作者提交 Markdown 草稿。
...
5.  **内容呈现与质量增强 (Content Rendering & Quality Enhancement)**:
    详细设计请参考: [内容呈现增强设计文档](../design/modules/rendering.md)

    -   **性能优化**: 实现 **Lighthouse 高分优化**，通过预加载、资源压缩等手段提升首屏加载性能。
...
9.  **系统固化与极致优化 (System Hardening & Refinement)**:
    -   **动态认证显示**: 自动检测环境变量配置，仅显示已配置 API Key 的第三方登录按钮。
    -   **管理员初始化逻辑**: 优化首次部署后的管理员账号确权流程（首选：第一个注册账号自动提权）。
...
4.  **数学公式支持 (Math Formula Support)**: 原生支持 $LaTeX$ 数学公式渲染，满足学术、算法等专业内容的展示需求。
5.  **评论权限精细化控制**: 支持单篇文章关闭评论、仅限注册用户评论、仅限订阅者评论等高级策略。
6.  **极客功能集成**: 支持 Markdown 代码块直接执行；集成基于 Git 的文章版本控制系统。
