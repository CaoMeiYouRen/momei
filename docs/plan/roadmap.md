...
7.  **多自定义主题与发布市场 (Theme Gallery & marketplace)**:
    -   **实现难度**: 中等 (Medium)。
...
10. **其他**
    -   **混合模式安装向导 (Hybrid Wizard)**: 针对生产环境提供 Web 端引导式配置，简化 SEO、AI 及邮件等高级服务的初始化。
    -   **私域引流**: 增加公众号关注引导、二维码展示等增强私域留存能力。
    -   **打赏模块**: 支持在文章末尾展示微信/支付宝打赏二维码，增加创作者激励。
    -   **文档全球化**: 完成面向用户的英文手册及 VitePress 国际化配置。
    -   **Hexo 一键迁移**: 开发 Hexo 到墨梅的一键转换脚本，支持文章、分类、标签的零损迁移。
