...
### 11. 页脚品牌与版权配置解耦 (Footer Branding & Copyright Configuration)
- **页脚品牌配置收敛**:
    - 将页脚版权展示从固定 i18n 文案调整为基于站点配置渲染，至少明确“站点名称 / 版权所有者”“版权起始年份”“Powered by 文案”三类输入来源。
    - 支持仅展示当前年份或“起始年份 - 当前年份”两类基础版权年份策略，站点名称默认沿用 `NUXT_PUBLIC_APP_NAME` / 公开站点设置，不在本轮额外引入品牌名多语言模型。
    - Powered by 段落由项目提供国际化模板，但需与站点自定义品牌字段解耦；同时梳理 `site_title` / `site_name` 与 `site_copyright` 的语义边界，避免把“文章默认版权协议”继续混用为页脚版权文案。
    - 若为页脚新增独立设置键或公开接口字段，必须同步补齐安装向导、后台设置、公开设置接口与组件测试，确保 ENV / DB / Default 口径一致。
