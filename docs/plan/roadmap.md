...
**目标**: 以“先守护、再收紧、再留痕”的顺序完成发布链路快收口，优先解决 CI 前置校验一致性与回归门禁可复盘问题。

**准入结论**: 本阶段共 `5` 条主线（`2` 条 P0 + `2` 条 P1 + `1` 条 P2），按“并行设计、串行落地”执行，容量受控且边界清晰。

**ROI 评估**: CI 前置守护脚本接入 `2.25`；发布链路最小回归闸门收紧 `2.00`；文档证据自动回填 `1.67`；守护策略分级（error/warn）治理 `1.60`；依赖风险日常任务与发布前守护口径对齐 `1.40`。
