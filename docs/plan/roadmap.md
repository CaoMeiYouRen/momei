...
**审计结论**: 阶段核心能力已在代码与文档中落地，包括设置解释层、AI 配额治理、Locale Registry、多语言 SEO、对象存储直传与分发链路补强；唯一未完成交付的是“存量资源链接重写与迁移工具”，已转入下一阶段继续推进。
...
5. **云端存储与静态资源优化 (Cloud Storage & Static Assets Optimization)**:
    ...
    - **静态资源统一管理**: 已收敛 CDN 前缀、对象路径前缀与业务默认目录模型；存量资源重写工具顺延到下一阶段。
...
1. **浏览器推送补强 (Browser Push Reinforcement) (P0)**:
    - **通知闭环**: 在现有 SSE 站内通知之上，补齐管理员站务通知与高价值异步任务完成提醒的浏览器侧触达能力。
    - **渐进式投递**: 引入 Service Worker、Push Subscription 与 VAPID 密钥管理，仅在用户离线或需要跨标签页触达时启用 Web Push；在线场景继续优先复用 SSE。
    - **场景约束**: 首轮仅聚焦管理员站务通知、AI / ASR / TTS 等异步任务完成提醒，避免过早泛化为全量通知系统。
    - **历史与审计补完**: 用户可查看历史通知列表；管理员可按渠道、类型、状态筛选通知投递记录，补齐“发出去了没有”的后台可观测性。
2. **代码质量与工程化 (Code Quality & Engineering) (P0)**:
    - **类型治理**: 聚焦核心路径消减 `any`、补齐共享类型与边界校验，优先覆盖通知、设置、编辑器与 AI 相关模块。
    - **复用与解耦**: 识别高重复的管理端列表、表单与服务逻辑，提炼共享 composables、utils 与 schema，避免继续膨胀。
    - **质量门禁强化**: 收紧 ESLint、TypeScript、i18n audit 与关键测试门禁，形成“新增功能必须过关”的持续治理机制。
    - **日期时间治理**: 明确 `useI18nDate`、`dayjs` 与 `new Date()` 的职责边界，收敛格式化、比较、区间判断等逻辑的实现口径，减少原生 `Date` 在业务逻辑中的分散使用。
    - **请求反馈国际化**: 收敛请求成功 / 失败后的提示链路，优先消费 i18n key 与稳定枚举映射，减少硬编码字段、后端原始 message 与未本地化文本直接展示给用户。
    - **定时任务节流**: 对友链可用性巡检等定时接口收敛默认频率、批量与退避策略，避免高频探测挤占运维预算。
3. **国际化与本地化增强 (I18n & L10n Enhancements) (P1)**:
    - **语言扩展**: 在 Locale Registry 基础上增加繁体中文与韩语支持，并按 readiness 分阶段启用。
    - **翻译治理**: 建立新增语言准入清单、术语约束与社区贡献流程，降低后续多语种扩展成本。
    - **回归保障**: 补齐新增语言在路由、SEO、邮件与核心 UI 链路上的回退和完整性校验。
    - **翻译语义纠偏**: 修正标签重译残留、分类显示语言错位与翻译版本媒体同步策略，明确 UI locale、文章 locale 与翻译簇之间的职责边界。
4. **用户体验优化 (User Experience Optimization) (P1)**:
    - **Demo 体验增强**: 丰富 Demo 预置内容、引导脚本和关键展示路径，让首次体验更接近真实使用场景。
    - **新手引导收口**: 优化 onboarding 的触发时机、步骤密度与多语言可理解性，减少打断感。
    - **反馈闭环**: 增加轻量反馈入口与分流机制，支持收集产品建议、Bug 反馈与体验问题，为后续迭代提供真实输入。
    - **初始化首配清单**: 重新梳理安装向导与 onboarding 的首配项，区分“建议立即配置”与“可稍后补全”的系统设置。
    - **管理列表预览增强**: 为文章管理页补齐封面预览与播客音频快速预览，降低后台核验成本。
...
### 7. 翻译工作流与分发解耦 (Translation Workflow & Distribution Decoupling)
- **可恢复全文翻译工作流**:
    - 为文章翻译提供来源版本选择、目标语言选择、字段范围选择与覆盖确认；来源文章限定在同一翻译簇内，覆盖草稿确认一次、覆盖已发布版本确认两次，避免高风险误覆盖。
    - 支持翻译中间态草稿、继续翻译、重新全文翻译，以及分类 / 标签按翻译簇映射与修复，降低多语言维护摩擦。
    - 为长文本翻译补齐基于 AITask / SSE 的进度回传，减少编辑器长时间无反馈的等待感。
- **第三方同步解耦**:
    - 将文章推送与 Memos 同步解耦，除发布时可选同步外，还应支持文章列表或编辑页中的手动同步、重试与幂等状态追踪。
    - 明确“首次同步 / 已同步重推 / 失败重试”的状态边界；对已同步文章，手动同步时要求用户在“更新原 Memo”与“重推新 Memo”之间二选一，避免发布副作用与外部分发策略耦合过深。

### 8. 编辑器兼容性与渐进替换评估 (Editor Compatibility & Incremental Replacement)
- **Markdown 编辑器兼容性预研**:
