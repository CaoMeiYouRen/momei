...
5.  **个性化生态进阶**:
    -   **主题画廊系统 (进行中)**: 支持保存多套自定义配置，建立主题预览与管理中心。
6.  **播客原生支持 (基础版)**:
...
2.  **精准推送控制**: 
    -   支持按分类/标签订阅特定内容，并提供日报/周报频率控制。
    -   **多格式集成订阅源 (Universal Feeds)**: 增加对 **Atom 1.0** 和 **JSON Feed 1.1** 的原生支持，并优化多格式下的 `enclosure` 兼容性。
3.  **商业化集成**: 集成 Stripe/支付宝支付接口，支持打赏功能或 Pro 会员套餐。
4.  **评论权限精细化控制**: 支持单篇文章关闭评论、仅限注册用户评论、仅限订阅者评论等高级策略。
5.  **极客功能集成**: 支持 Markdown 代码块直接执行；集成基于 Git 的文章版本控制系统。
6.  **多自定义主题与发布市场 (Theme Gallery & marketplace)**:
    -   **实现难度**: 中等 (Medium)。
    -   **数据模型**: 新增 `themes` 表，将原本平铺在 `settings` 表里的主题配置迁移到独立的记录中，包含 `config` (JSON), `author_id`, `is_published` 等字段。
    -   **管理流程**: 构建一套完整的主题 CRUD 接口，以及“激活主题”的逻辑（主设置中仅需记录当前 `theme_id`）。
    -   **UI 交互**: 设计“主题库/画廊”界面用于管理和切换，类似于 VS Code 的主题切换或 WordPress 的主题管理体验。    
    - **权重权衡 (Specificity Balancing)**: 随着全站禁用 `!important` 规范的引入，需探索通过增强 CSS 变量系统或自动化 CSS Layer (`@layer`) 策略，平衡系统默认样式与用户自定义样式之间的覆盖关系。  
    -   **安全性**: 实现针对发布主题的自动化 XSS 扫描和人工审核机制，确保名称、元数据及配置中不包含恶意代码。
    -   **扩展性**: 基于现有的 `theme_preset` 架构实现平滑转换，支持从单套自定义迁移到多套保存。
7.  **播客创作进阶**:
    -   **音频文章与播放器**: 集成 AI TTS 自动生成文章朗读版，配合全站悬浮播放器实现断点续播。
8.  **高级 AI 创作流 (Advanced AI Creative Flow)**:
    -   **灵感引擎 (Inspiration Engine)**: 实现“移动端->后台”的灵感快捕工作流，支持 API/Webhook 高效收集碎片化想法。
    -   **脚手架式写作助手**: AI 不直接生成正文，而是提供逻辑提纲、论点建议及“辩论式”引导，辅助作者挖掘深度。
9.  **其他**
    -   **私域引流**: 增加公众号关注引导、二维码展示等增强私域留存能力。
    -   **打赏模块**: 支持在文章末尾展示微信/支付宝打赏二维码，增加创作者激励。
    -   **文档全球化**: 完成面向用户的英文手册及 VitePress 国际化配置。
    -   **权限安全校验**: 彻底排查并修复特定页面（如墨梅页面等）的权限逻辑漏洞。
    -   **感官体验增强**: 实现看板娘、背景粒子等可选的视觉增强开关。
## 4. 技术架构与质量保证
