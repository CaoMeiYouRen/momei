...
### 7. 云端资源迁移与链接重写 (Cloud Asset Migration & Rewrite)
- **存量资源链接重写工具**:
    - 为历史文章中的绝对 URL 提供 dry-run、域名过滤、批量重写与审计报告，降低存储后端切换成本。
    - 优先评估 `scripts/storage/rewrite-asset-urls.ts` 或受管理员权限保护的后台入口，避免资源迁移只能依赖手工 SQL 或内容层逐篇修订。

### 8. 第三方分发解耦与投递控制 (Content Distribution Decoupling)
- **Memos / Wechatsync 手动同步与重试**:
    - 将文章推送与第三方同步解耦，除发布时可选同步外，还应支持文章列表或编辑页中的手动同步、重试与幂等状态追踪。
    - 明确“首次同步 / 已同步重推 / 失败重试”的状态边界；对已同步文章，手动同步时要求用户在“更新原 Memo”与“重推新 Memo”之间二选一，避免发布副作用与外部分发策略耦合过深。

### 9. 编辑器兼容性与渐进替换评估 (Editor Compatibility & Incremental Replacement)
- **Markdown 编辑器兼容性预研**:
    - 单独评估 `mavon-editor` 在 Nuxt 4 / Vue 3 场景下的运行时异常、样式注入与暗色模式适配根因，先定位是组件实现问题、依赖导出问题还是资源注入问题。
    - 若需替换现有编辑器，必须先建立功能矩阵与回归清单，至少覆盖工具栏、双栏预览、全屏、图片上传回填、快捷插入与现有页面级样式兼容，避免在常规缺陷修复迭代中直接整体替换。
