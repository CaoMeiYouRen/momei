...
### 第四十一阶段：TypeORM 前置清障与治理切片推进 (TypeORM Pre-Clearance & Governance Slices)

**时间表**: 2026-06-01 ~ 约 1 - 2 周
**目标**: 在第四十阶段完成发布前守护与 TypeORM 评估归档后，以“0 个新功能 + 5 个优化”的受控组合推进下一阶段，优先把 TypeORM 升级阻断项前置清障，并同步收敛 Postgres 热点读链路、文档门禁 / 脚本治理、结构复用与 ESLint / 类型债，形成可复跑、可量化、可继续上收的治理闭环。
...
1. **主线：TypeORM 升级前置清障切片 (P0)**:
    - **执行范围**: 按第四十阶段 `NO-GO（直接升级）` 结论，先处理适配层契约、测试桩形态与查询链路兼容缺口，形成可复跑清障清单。
    - **非目标**: 不直接升级 `typeorm` 到 `1.0.0`，不把评估态结论误写为实施完成。
    - **最小验收**: 产出阻断项分桶、优先级、回滚锚点与下次复评触发条件。
    - **验证与证据**: TypeORM 评估文档增量、定向验证记录与活动回归窗口摘要。

2. **主线：Postgres 热点读链路治理 (P0)**:
    - **执行范围**: 聚焦 `posts / archive / categories / tags / settings / friend-links` 的单路径或单组切片，优先收敛热点读结果集体量与重复读。
    - **非目标**: 不并行扩写为全站数据库重构，不与非热点链路绑定推进。
    - **最小验收**: 至少一组热点读链路完成字段裁剪、缓存或去重策略，并能给出 calls / rows / 网络体量对比。
    - **验证与证据**: `pg_stat_statements` 或等价 live sample、定向测试与回归记录。
...
4. **主线：结构复用治理 (P1)**:
    - **执行范围**: 按“重复代码 + 零散类型 + 纯函数 / 工具函数”口径继续做小切片，优先处理可量化收益热点。
    - **非目标**: 不推动跨目录大重构，不为复用而复用。
    - **最小验收**: 至少完成 1 组热点切片，且 `duplicate-code` 基线不反弹。
    - **验证与证据**: `duplicate-code` 基线对比、定向测试与复用边界说明。

5. **主线：ESLint / 类型债治理 (P1)**:
    - **执行范围**: 继续坚持“单规则 + 单文件 / 双文件”窄切片，结合规则债 inventory 输出推进清偿。
    - **非目标**: 不扩写为全仓 `any` 清零或多规则并行大爆炸治理。
    - **最小验收**: 目标规则命中数下降并通过定向 lint / typecheck。
    - **验证与证据**: 规则债 baseline / delta、定向校验记录与回归窗口摘要。
