...
> 本区基于 `roadmap.md`、`todo-archive.md`、`CHANGELOG.md` 与当前源码 / 文档事实源整理，用于 backlog 背景判断与后续切片参考；第三十一、第三十二阶段已完成审计归档，当前仍以中文 `roadmap.md`、`todo.md` 与 `todo-archive.md` 为唯一规划事实源。
...
11. **微信公众号格式编辑器与预览支持**
- **留档范围**:
    - 为后台文章编辑链路补一条“微信公众号格式”导出 / 编辑 / 预览候选能力，重点关注排版样式、段落层级、图片与引用块等在公众号阅读语境中的展示一致性。
    - 本条默认仅评估“公众号风格预览 / 导出辅助”与现有 Markdown 编辑链路的协同方式，不直接承诺整体替换当前 `mavon-editor`，也不把范围扩写为新的富文本编辑器工程。
- **当前结论**:
    - 该需求与第二十五、第二十八阶段已完成的“编辑器稳定性 / Markdown 与外观一致性收口”相邻，但不属于当前已交付能力的自然尾项，应作为新的候选能力单独评估。
    - 由于它会同时触达编辑器包装层、预览渲染、分发 / 导出链路以及 WechatSync / 公众号使用场景，进入实现前必须先补齐专项设计文档，明确“公众号格式”究竟指公众号图文排版预览、导出模板，还是面向 WechatSync 的投递前格式化辅助，避免需求被最小化解释。
- **后续上收条件**:
    - 已明确首版范围：只做“公众号风格预览 / 一键格式化辅助 / 导出模板”中的哪一类，哪些能力明确不做。
    - 已明确最小验证矩阵，至少覆盖标题层级、图片、引用块、代码块 / 提示容器、长文分页感知，以及编辑器预览与最终导出结果的一致性。
...
13. **前端直出 TTS + 直传 OSS：解决云函数 TTS 超时** — **已交付（第三十四—三十五阶段）**

- **交付状态**: Volcengine speech（HTTP 单向流式）+ podcast（WebSocket 双向流式）前端直连已完成最小闭环。JWT 凭证下发（`POST /api/ai/tts/credentials`）→ 前端直连 composable（`use-tts-volcengine-direct.ts`）→ OSS 直传（`useUpload`）→ 元数据回写（`PUT /api/posts/[id]/tts-metadata`）全链路已落地并覆盖定向测试。
- **设计文档**: `docs/design/governance/tts-frontend-direct-evaluation.md`
- **非目标守线**: 未重写 `TTSService.processTask()`，未扩展到 AI Image，未改变 `media-task-monitor` 补偿逻辑。
- **归档阶段**: 第三十四阶段（评估与原型）、第三十五阶段（计量口径与防回归）。本条从 backlog 移除，不再保留为活跃候选。
