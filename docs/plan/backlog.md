...
1. **商业化转型可行性重评（付费增强功能优先）**
- **目标**:
    - 在 `1` 个月决策窗口内重新判断墨梅是否值得继续推进商业化转型。
    - 把主评估对象收敛为“开源核心 + 付费增强功能”，同时将赞助 / 会员降级为辅助收入口径。
    - 明确当前缺口究竟是“产品主卖点不清晰”，还是“承接层与成交动作不足”。
- **约束**:
    - 继续保持开源核心，不把免费核心整体闭源化。
    - 保持单人可持续维护，不引入重交付、重咨询或高售后路径。
    - 目标用户仍收敛在技术作者 / 独立开发者，不扩张到陌生客群。
    - 广告联盟不纳入本轮评估范围，也不作为当前主变现方向。
- **执行范围**:
    - 比较三种结论路径：付费增强功能、赞助 / 会员、暂缓商业化。
    - 盘点现有商业化基础能力是否足以支撑主卖点与最小成交路径。
    - 核对当前公开站点、Demo 与项目说明页是否已具备统一承接层。
    - 输出一份可复用的验收标准、最小验证矩阵与上收条件。
- **非目标**:
    - 不直接实现支付接入、会员中心、价格页或正式销售页。
    - 不在本轮继续评估广告联盟、流量广告、托管版 / SaaS 或重服务模式。
    - 不把本轮候选分析直接升格为正式开发阶段。
- **验收标准**:
    - 已明确本轮主问题、目标用户、约束条件与排除项。
    - 已给出付费增强功能、赞助 / 会员、暂缓商业化三条路径的比较口径。
    - 已明确广告联盟不进入本轮评估。
    - 已能说明当前最主要的商业化阻塞点位于主卖点、承接层还是付费信号。
    - 已补齐最小验证矩阵与正式上收条件。
    - 已形成专项文档并作为候选分析的唯一事实源。
- **最小验证矩阵**:
    - 范围治理：`docs/design/governance/commercialization-reassessment-framework.md` 与本条候选保持一致。
    - 历史能力盘点：`docs/guide/features.md`、`docs/design/modules/commercial.md`、`docs/design/modules/demo-mode.md` 可支撑当前基础判断。
    - 承接入口核对：`pages/index.vue`、`components/demo-banner.vue`、`pages/about.vue` 已完成当前入口事实核对。
    - 规划同步：本条候选已具备执行范围、非目标、验收标准与上收条件。
- **专项文档**:
    - [商业化转型可行性重评框架](../design/governance/commercialization-reassessment-framework.md)

2. **`caomei-auth` 第三方登录支持评估与接入预研**
- **协议与能力基线**:
