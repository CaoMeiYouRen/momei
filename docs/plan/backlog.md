...
- **当前状态**:
    ...
    - 第三十二阶段已正式上收下一轮组合切片，并把范围继续冻结为“公共页模板片段 + 列表型查询 helper”，避免复用治理外溢成跨目录重构。
- **最近一次上收阶段**:
    - 第三十二阶段（规划中）。
- **下一次可切片方向**:
...
- **当前状态**:
    ...
    - 第三十二阶段已正式上收下一轮切片，但明确要求本轮只能从“一组请求入口”或“一组公开热点读链路”中二选一推进，不再并行开启两类方向。
- **最近一次上收阶段**:
    - 第三十二阶段（规划中）。
- **下一次可切片方向**:
...
- **状态说明**:
    - 已上收至第三十二阶段正式规划，当前只保留“多语言内容资产化增强包的统一承接入口”这一条执行面。
    - 具体执行范围、非目标与最小验证矩阵以 `roadmap.md` / `todo.md` 中的第三十二阶段为准；backlog 不再重复维护完整实施正文。
