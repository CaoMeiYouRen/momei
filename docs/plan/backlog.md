...
9. **TypeORM 1.0.0 升级评估（已上收至第四十阶段）**
- **历史说明**:
    - 本条已于 `2026-05-23` 上收到 [项目计划](./roadmap.md) 第四十阶段与 [待办事项](./todo.md) 当前执行面。
    - 专项评估文档已落地至 [typeorm-v1-upgrade-assessment.md](../design/governance/typeorm-v1-upgrade-assessment.md)，后续仅保留历史索引，不再在 backlog 重复维护实施正文。

10. **CI 前置守护脚本接入（已上收至第四十阶段）**
- **历史说明**:
    - 本条已于 `2026-05-23` 上收到 [项目计划](./roadmap.md) 第四十阶段与 [待办事项](./todo.md) 当前执行面。
    - 后续若需要扩展到更多 workflow 或升级为跨仓统一门禁，再作为新的短期候选单独评估。
