...
8. **微信公众号格式预览 / 导出辅助（已上收至第三十九阶段）**
- **历史说明**:
    - 本条已于 `2026-05-19` 正式上收至 [项目计划](./roadmap.md) 第三十九阶段与 [待办事项](./todo.md) 当前执行面，不再作为 backlog 候选重复维护。
    - 后续若首版交付后仍有“完整编辑器替换”“公众号图床整合”“更深的一键排版模板化”等扩展需求，再作为新的短期候选单独评估，而不是回写本条旧候选正文。

9. **TypeORM 1.0.0 升级评估（源自 Dependabot #423）**
- **需求背景**:
    - Dependabot 提交了 `typeorm` 从 `0.3.29` 升级到 `1.0.0` 的 PR（大版本变更，含 breaking changes）。
    - 当前诉求是先做升级评估，不直接承诺本阶段落地升级实现。
- **范围判定**:
    - 当前阶段 `todo.md` 仍有未收口事项，且该需求不属于当前条目的既有验收范围，默认按 backlog 候选管理。
    - 若评估过程中确认存在当前依赖链 `high+` 安全风险且无法接受延期，再按 B 类插队规则回灌当前阶段。
- **评分（Momei 评估矩阵）**:
    - Value: `4`（数据库核心依赖，影响面广）
    - Alignment: `4`（契合长期稳定性与依赖治理主线）
    - Difficulty: `4`（大版本 API/行为变化，涉及 TypeORM 适配层与测试桩）
    - Risk: `4`（认证、查询构造、实体映射与迁移链路存在回归风险）
    - Score: `(4 + 4) / (4 + 4) = 1.00` → 进入 backlog 候选，暂不直接上收当前阶段。
- **前置产物（评估完成定义）**:
    - 输出一份专项评估文档（建议落点：`docs/design/governance/typeorm-v1-upgrade-assessment.md`）。
    - 明确破坏性变更清单、受影响代码分桶、升级路径（直接升级 / 分阶段 / 暂缓）与回滚策略。
    - 给出风险分级和 go/no-go 结论，并标注是否触发 B 类插队条件。
- **最小验证矩阵（仅评估阶段）**:
    - 影响面盘点：`server/database/**`、`server/entities/**`、`server/api/**`、`server/services/**`、`lib/auth.ts` 与 TypeORM 相关测试桩。
    - 基线检查：`pnpm run typecheck`、定向 Vitest（数据库适配层与认证链路），以及依赖安全审计结果对账。
    - 证据落点：`docs/reports/regression/current.md`（评估记录摘要）+ 评估文档中的引用链接。
- **非目标**:
    - 本条不直接执行 TypeORM 升级改造、不承诺本阶段合并 Dependabot PR、不并行扩写 ORM 替换工程。
