...
- **当前状态**:
    ...
    - 第二十九阶段已正式上收新的规则收紧切片，当前优先处理 `no-misused-spread` 一类命中有限、回滚边界清晰的规则族，并择机补入另一条命中可控的规则；不直接扩写到 `no-unsafe-*` 或全仓 `any` 清零工程。
- **最近一次上收阶段**:
    - 第二十九阶段（执行中）。
- **下一次可切片方向**:
...
- **当前状态**:
    ...
    - 第二十九阶段已正式上收新的复用收敛切片，当前热点聚焦前台 legal pages、categories / tags 公共页，以及查询参数处理与列表读模型组装等剩余重复区域。
- **最近一次上收阶段**:
    - 第二十九阶段（执行中）。
- **下一次可切片方向**:
...
- **当前状态**:
    ...
    - 第二十九阶段已正式上收本轮文档治理组合切片，当前优先把明确与 Todo 强绑定、或明显脱离当前实现的治理文档改成更短的 delta 版本，再评估是否应下线旧正文。
- **最近一次上收阶段**:
    - 第二十九阶段（执行中）。
- **下一次可切片方向**:
...
- **当前状态**:
    ...
    - 第二十九阶段已正式上收本轮治理切片，当前优先完成活动窗口、归档入口与兼容说明的进一步收敛，再按模块分片与日期分片逐步迁移正文。
- **最近一次上收阶段**:
    - 第二十九阶段（执行中）。
- **下一次可切片方向**:
...
- **当前状态**:
    ...
    - 第二十九阶段已正式上收本轮评估切片，当前先定义压力阈值、拆分策略与归档前后对照指标，再决定是否进入更大范围搬迁。
- **最近一次上收阶段**:
    - 第二十九阶段（执行中）。
- **下一次可切片方向**:
...
- **当前状态**:
    ...
    - 第二十八阶段已完成运行时治理首轮切片；第二十九阶段已正式上收下一轮治理，当前重点扩到 missing blocker 分级、unused 字段排查与共享命名空间继续收敛。
- **最近一次上收阶段**:
    - 第二十九阶段（执行中）。
- **下一次可切片方向**:
...
> 本区基于 `roadmap.md`、`todo-archive.md`、`CHANGELOG.md` 与当前源码 / 文档事实源整理，用于 backlog 背景判断与后续切片参考；第二十九阶段已正式开启，当前中文 `roadmap.md` 与 `todo.md` 为执行事实源，上一阶段收口结论继续以 `todo-archive.md` 与 `docs/reports/regression/current.md` 为准。
...
- **方向判断**:
    - 第二十九阶段已按“1 个新功能 + 5 个优化”的组合正式开启执行面。
    - 当前新增能力聚焦评论区翻译；优化主线聚焦 GEO / SEO 可见性、ESLint / 类型债、重复代码复用、国际化第二轮治理，以及文档事实源 / 回归记录 / 深度归档治理。
    - backlog 继续承担未上收候选与长期主线后续切片的留档职责，避免当前阶段执行面再次膨胀。
...
### 本轮新增候选处理结果（2026-04-20）

- 已上收至 [roadmap.md](./roadmap.md) 第二十九阶段（执行中）：
    - 评论区翻译功能。
    - GEO / SEO / AI crawler 可见性与文章可引用性补强。
...
8. **Hexo 风格文章仓库同步（GitHub / Gitee）能力评估与候选落地**
- **候选目标**:
