...
- **方向判断**:
    ...
    - 第三十三阶段已按"1 个新功能 + 4 个优化"的组合完成并审计归档。第三十四阶段已按"1 个新功能评估 + 5 个优化"正式规划上收：新功能为前端直出 TTS + 直传 OSS 评估与原型，优化为 coverage 80%+ 冲刺、周期性回归执行、ESLint 下一轮切片、i18n 运行时继续扩面、文档翻译 freshness 续。具体执行范围与验收标准以 `roadmap.md` / `todo.md` 第三十四阶段为准。
