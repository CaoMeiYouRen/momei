...
> 本区基于 `roadmap.md`、`todo-archive.md`、`CHANGELOG.md` 与当前源码 / 文档事实源整理，用于 backlog 背景判断与后续切片参考；第二十九阶段已完成审计归档，当前暂无已正式写入的下一阶段执行面，后续仍以中文 `roadmap.md`、`todo.md` 与 `todo-archive.md` 为唯一规划事实源。
...
- **方向判断**:
    ...
    - 最近一轮新增能力聚焦评论区翻译；优化主线聚焦 GEO / SEO 可见性、ESLint / 类型债、重复代码复用、国际化第二轮治理，以及文档事实源 / 回归记录 / 深度归档治理。
    - backlog 继续承担未上收候选与长期主线后续切片的留档职责；下一阶段在正式写入前，仍先停留在候选分析与准入评估。
...
- 已上收至 [roadmap.md](./roadmap.md) 第二十九阶段（已审计归档）：
    - 评论区翻译功能。
