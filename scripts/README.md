...
| `governance` | `pnpm governance:check:scripts` / `pnpm governance:audit:simple-duplicates` / `pnpm governance:audit:eslint-debt` / `pnpm governance:audit:comment-drift` / `pnpm governance:count:primevue-usage`（另有维护型入口 `:components`，用于升级 primevue 主版本后重生成组件名清单） | 默认输出 `artifacts/governance/*.json` + `.md`；可选 `--output`、`--root=<dir>` | 脚本目录健康体检、简单重复候选、ESLint / 类型债分桶、注释漂移候选与 PrimeVue 迁移用量基线的 baseline | `check:scripts` 与三个 audit 入口均已进入 weekly warning 基线（不直接阻断回归）；docs candidate 入口保持独立只读 baseline | 正式入口 |
...
| `scripts/governance/` | `check-script-governance.mjs`、`audit-simple-duplicates.mjs`、`audit-eslint-debt.mjs`、`audit-comment-drift.mjs`、`count-primevue-usage.mjs` | `pnpm governance:check:scripts`、`pnpm governance:audit:simple-duplicates`、`pnpm governance:audit:eslint-debt`、`pnpm governance:audit:comment-drift`、`pnpm governance:count:primevue-usage`、`pnpm regression:weekly`（4 个 governance 步骤） | 产出脚本稳定入口、临时残留、文档漂移、简单重复候选、ESLint / 类型债分桶、注释漂移候选与 PrimeVue 迁移用量基线的 JSON / Markdown baseline | 新增保留 |
